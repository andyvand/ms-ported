...
FindMetadataFromChildren( HWND hChildWnd, LPARAM lParam )
{
    ...
    if (IsWindowUnicode( hChildWnd )) {
        ...
        CharsCopied = GetClassNameA( hChildWnd, (PCHAR)szTextA, sizeof( szTextA ) / sizeof( CHAR ));
        ...
        if (CharsCopied == 0 || CharsCopied >= (sizeof( szTextA ) / sizeof( CHAR ))) {
            return TRUE;
        }
        ...
        if (strcmp( (PCHAR)szTextA, DIALOGRESIZEDATACLASSA ) == 0) {
            SendMessage( hChildWnd, WM_GETDIALOGMETADATA, 0, lParam );
            return FALSE;
        }
    }
...
ProcessResizeOnChildren( HWND hChildWnd, LPARAM lParam )
{
    ...
    if (IsWindowUnicode( hChildWnd )) {
        ...
        if (strcmp( (PCHAR)szTextA, DIALOGRESIZECONTROLCLASSA ) == 0) {
            SendMessage( hChildWnd, WM_RESIZEPARENT, 0, lParam );
        }
    }
