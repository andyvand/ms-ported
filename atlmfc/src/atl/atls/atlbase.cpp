...
bool CAtlBaseModule::AddResourceInstance(_In_ HINSTANCE hInst)
{
...
bool CAtlBaseModule::RemoveResourceInstance(_In_ HINSTANCE hInst)
{
	CComCritSecLock<CComCriticalSection> lock(m_csResource, false);
	if (FAILED(lock.Lock()))
	{
...
HINSTANCE CAtlBaseModule::GetHInstanceAt(_In_ int i)
{
	CComCritSecLock<CComCriticalSection> lock(m_csResource, false);
	if (FAILED(lock.Lock()))
	{
