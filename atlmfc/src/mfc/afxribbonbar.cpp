...
BOOL CMFCRibbonApplicationButton::SetACCData(CWnd* pParent, CAccessibilityData& data)
{
	...
	{
		data.m_strAccName = _T("Application menu");
	}
	else
	{
		data.m_strAccName = m_strText;
	}
...
void CMFCRibbonBar::RecalcLayout()
{
	...
	else
	{
		...
		if (GetGlobalData()->IsDwmCompositionEnabled())
		{
			...
			int nSysButtonsWidth = 3 * ncm.iCaptionWidth + GetSystemMetrics(SM_CXSIZE);
	...
	if (GetGlobalData()->IsDwmCompositionEnabled() && m_bReplaceFrameCaption)
	{
		...
		if (_AfxDwmExtendFrameIntoClientArea(GetParent()->m_hWnd, &margins) == S_OK)
		{
...
BOOL CMFCRibbonBar::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	if (m_pToolTip->m_hWnd == NULL || pNMH->hwndFrom != m_pToolTip->m_hWnd)
	{
		return FALSE;
	}
...
BOOL CMFCRibbonBar::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN)
	{
		...
		case VK_SPACE:
			if (m_nKeyboardNavLevel >= 0 && CMFCPopupMenu::GetActiveMenu() == NULL && ::GetFocus() == m_hWnd)
			{
				DeactivateKeyboardFocus();
...
void CMFCRibbonBar::ShowSysMenu(const CPoint& point)
{
	...
	if (pParentWnd->m_hWnd != NULL)
	{
		...
		{
			::TrackPopupMenu(pMenu->GetSafeHmenu(), TPM_CENTERALIGN | TPM_LEFTBUTTON, point.x, point.y, 0, GetOwner()->m_hWnd, NULL);
		}
	}
}
...
void CMFCRibbonBar::SetMaximizeMode(BOOL bMax, CWnd* pWnd)
{
	...
	if (bMax)
	{
		...
		if (hSysMenu != NULL)
		{
			...
			{
				m_TabElements.AddButton(new CMFCRibbonCaptionButton(SC_MINIMIZE, pWnd->m_hWnd));
				m_nSystemButtonsNum++;
			}
			...
			{
				m_TabElements.AddButton(new CMFCRibbonCaptionButton(SC_RESTORE, pWnd->m_hWnd));
				m_nSystemButtonsNum++;
			}

			// Add a close box:
			CMFCRibbonCaptionButton* pBtnClose = new CMFCRibbonCaptionButton(SC_CLOSE, pWnd->m_hWnd);
			...
			if (hSysMenu != NULL)
			{
				...
				if (!::GetMenuItemInfo(hSysMenu, SC_CLOSE, FALSE, &menuInfo) || (menuInfo.fState & (MFS_GRAYED | MFS_DISABLED)))
				{
...
void CMFCRibbonBar::SetActiveMDIChild(CWnd* pWnd)
{
	...
	for (int i = 0; i < (int)m_TabElements.m_arButtons.GetSize(); i++)
	{
		...
		if (pCaptionButton != NULL)
		{
			...
			pCaptionButton->m_hwndMDIChild = pWnd->m_hWnd;
		}
	}
}
...
LRESULT CMFCRibbonBar::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		LRESULT lResult = 0;
		_AfxDwmDefWindowProc(GetParent()->m_hWnd, message, wParam, lParam, &lResult);
...
void CMFCRibbonBar::SetKeyboardNavigationLevel(CObject* pLevel, BOOL bSetFocus)
{
	...
	BOOL bActive = (pFocus->m_hWnd != NULL &&
		(pParentFrame->IsChild(pFocus) || pFocus->m_hWnd == pParentFrame->m_hWnd));
...
#pragma warning(push)
#pragma warning(disable:6214)	// While the documentation and implementation completely doesn't make sense,
								// we cannot change this behavior for now.
...
HRESULT CMFCRibbonBar::accLocation(long *pxLeft, long *pyTop, long *pcxWidth, long *pcyHeight, VARIANT varChild)
{
	if (m_bSingleLevelAccessibilityMode)
	{
		return accLocation(pxLeft, pyTop, pcxWidth, pcyHeight, varChild);
	}
...
void CMFCRibbonBar::ShowKeyTips(BOOL bRepos)
{
	...
	if (m_pToolTip->m_hWnd != NULL && m_pToolTip->IsWindowVisible())
	{
		m_pToolTip->SetWindowPos(&wndTopMost, -1, -1, -1, -1, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
	}
}
...
void CMFCRibbonBar::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	if (!bShow && m_bIsTransparentCaption)
	{
		...
		_AfxDwmExtendFrameIntoClientArea(GetParent()->m_hWnd, &margins);
	}
}
