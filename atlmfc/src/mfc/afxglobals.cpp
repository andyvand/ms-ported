...
CMemDC::~CMemDC()
{
	...
	{
		m_dcMem.Detach();
		_AfxEndBufferedPaint((HPAINTBUFFER)m_hBufferedPaint, TRUE);
	}
	else if (m_bMemDC)
	{
		//--------------------------------------
...
void AFX_GLOBAL_DATA::UpdateFonts()
{
	...
	// Create vertical, dialog underline and dialog bold fonts:
	CFont font;
...
BOOL AFX_GLOBAL_DATA::DrawParentBackground(CWnd* pWnd, CDC* pDC, LPRECT rectClip)
{
	...
	bRes = DrawThemeParentBackground(pWnd->m_hWnd, pDC->GetSafeHdc(), rectClip) == S_OK;
...
CString AFX_GLOBAL_DATA::RegisterWindowClass(LPCTSTR lpszClassNamePrefix)
{
	...
	// We do not want to change the output format and size of the output values for backward compatibility.
	// There can be truncation of output texts of handle values if they are greater than maximum integer.
	// But using %x will allow the resultant class names for x86 and x64 versions be the same.
...
HCURSOR AFX_GLOBAL_DATA::GetHandCursor()
{
	if (m_hcurHand == NULL)
	{
        LPCWSTR Hand = (LPCWSTR)((ULONG_PTR)IDC_HAND);
		m_hcurHand = ::LoadCursorW(NULL, Hand);
	}
