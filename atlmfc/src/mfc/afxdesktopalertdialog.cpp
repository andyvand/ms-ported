...
BOOL CMFCDesktopAlertDialog::HasFocus() const
{
	...
	BOOL bActive = (pFocus->m_hWnd != NULL && (IsChild(pFocus) || pFocus->m_hWnd == m_hWnd));
...
BOOL CMFCDesktopAlertDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_btnURL.m_hWnd == (HWND) lParam && m_btnURL.GetDlgCtrlID() == LOWORD(wParam) && m_pParentPopup->OnClickLinkButton(m_btnURL.GetDlgCtrlID()))
	{
		return TRUE;
	}
...
BOOL CMFCDesktopAlertDialog::OnInitDialog()
{
	...
	while (pWndChild != NULL)
	{
		...
		else
		{
			...
			::GetClassName(pWndChild->m_hWnd, lpszClassName, AFX_MAX_CLASS_NAME);
...
{
	if (m_bDontSetFocus && pOldWnd->m_hWnd != NULL)
	{
		pOldWnd->SetFocus();
		return;
	}
...
{
	if (pMsg->message == WM_LBUTTONDOWN && m_pParentPopup->m_hWnd != NULL)
	{
		...
		if (pWnd != NULL)
		{
			...
			::GetClassName(pWnd->m_hWnd, lpszClassName, AFX_MAX_CLASS_NAME);
			CString strClass = lpszClassName;

			if (strClass == AFX_STATIC_CLASS || pWnd->m_hWnd == m_hWnd)
			{
