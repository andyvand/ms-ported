...
void RFX_Text(_In_ CFieldExchange* pFX, LPCTSTR szName,
	_In_reads_(nMaxLength) CharType* value, int nMaxLength,
	int nColumnType, short nScale, short nFieldType,
	CharType cSpace)
{
	...
	case CFieldExchange::BindFieldToColumn:
		{
            ...
            // Changing this legacy code to fix the warning was deemed too risky by the libsdev team.
            // Suppressing instead.
...
template<typename CharType>
inline void RFX_Text_Bulk(_In_ CFieldExchange* pFX, LPCTSTR szName,
	_When_(pFX->m_nOperation == CFieldExchange::AllocMultiRowBuffer, _Outptr_result_maybenull_)
	_When_(pFX->m_nOperation == CFieldExchange::DeleteMultiRowBuffer, _Pre_notnull_ _At_(*prgLengths, _Post_invalid_))
	CharType** prgStrVals,
	_When_(pFX->m_nOperation == CFieldExchange::AllocMultiRowBuffer, _Outptr_result_maybenull_)
	_When_(pFX->m_nOperation == CFieldExchange::DeleteMultiRowBuffer, _Pre_notnull_ _At_(*prgLengths, _Post_invalid_))
	LONG_PTR** prgLengths,
	int nMaxLength, short nFieldType)
{
	ENSURE_ARG(AfxIsValidAddress(pFX, sizeof(CFieldExchange)));
	ENSURE_ARG(AfxIsValidString(szName));
...
void AFXAPI RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	_Outptr_result_buffer_maybenull_(nMaxLength) LPWSTR *prgStrVals, LONG_PTR** prgLengths, int nMaxLength)
{
	RFX_Text_Bulk(pFX, szName, prgStrVals, prgLengths, nMaxLength, SQL_C_WCHAR);
}

void AFXAPI RFX_Text_Bulk(CFieldExchange* pFX, LPCTSTR szName,
	_Outptr_result_buffer_maybenull_(nMaxLength) LPSTR *prgStrVals, LONG_PTR** prgLengths, int nMaxLength)
{
