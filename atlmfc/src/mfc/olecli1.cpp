...
//
void COleClientItem::GetItemName(_Out_writes_z_(OLE_MAXITEMNAME) LPTSTR lpszItemName) const
{
	ASSERT_VALID(this);
	...
	UINT& nIndex)
{
	...
	*pstrTarget = '\0';
	...
	if (*pstrSource != '\0')
	{
		LPTSTR pstrIndex = (LPTSTR)_tcsinc(pstrSource);
...
void COleClientItem::GetItemStorageFlat()
{
	...
	ASSERT(m_lpLockBytes != NULL);
	_Analysis_assume_(m_lpLockBytes != NULL);
			...
			case OLEGETMONIKER_FORCEASSIGN:
				{
					...
					CStringW strItemName(szItemName);
					// *ppMoniker will be set to null if an error has occurred; 
					// NOTE that *ppMoniker is checked for nullness in the if statement below
					//	to detect whether an error has occurred. 
					(void) ::CreateItemMoniker(OLESTDDELIMOLE, strItemName.GetString(), ppMoniker);
