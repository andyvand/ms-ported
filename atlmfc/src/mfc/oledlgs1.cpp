						   ...
						   ULONG* cchEaten, LPMONIKER* plpmkOut)
{
	...
	else
	{
		...
		*cchEaten = cEaten;
		*lpszDest = '\0'; // null-terminate the string
...
_Use_decl_annotations_ STDMETHODIMP COleUILinkInfo::SetLinkSource(
						...
						KEY_READ, &hkItem) == ERROR_SUCCESS)
					{
							...
							KEY_READ, &hkDocObject) == ERROR_SUCCESS))
						{
								...
								KEY_READ, &hkDocObject) != ERROR_SUCCESS ) )
							{
								CLSID clsid;
								CStringW ws(szName);
								HRESULT hr = CLSIDFromString((LPOLESTR)ws.GetString(), &clsid);
								if (hr == NOERROR) {
...
COleConvertDialog::COleConvertDialog(_In_ COleClientItem* pItem, DWORD dwFlags,
