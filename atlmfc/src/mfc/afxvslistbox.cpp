...
{
	if (m_hWnd == NULL || m_pWndList == NULL)
	{
		return -1;
	}
...
BOOL CVSListBox::EditItem(int iIndex)
{
	...
	m_wndEdit.SubclassWindow(pEdit->m_hWnd);
...
{
	if (pMsg->message == WM_LBUTTONDOWN && m_pWndList != NULL && m_pWndList->GetEditControl() == NULL && m_pWndList->m_hWnd == CWnd::GetFocus()->m_hWnd)
	{
		ASSERT_VALID(m_pWndList);
