...
#define _imp_DefWindowProc DefWindowProcW
...
#else
#if defined(_X86_) && !defined(_M_HYBRID)
#define _imp_DefWindowProc DefWindowProcA
#elif defined(_M_HYBRID)
#define _imp_DefWindowProc DefWindowProcW
#else
#define _imp_DefWindowProc DefWindowProcA
...
extern "C" {
	LRESULT (CALLBACK *_callback_DefWindowProc)(_In_ HWND hWnd, _In_ UINT Msg, _In_ WPARAM wParam, _In_ LPARAM lParam);
}
...
AFX_STATIC HWND AFXAPI _AfxGetParkingWindow()
{
	...
	return pThreadState->m_pWndPark->m_hWnd;
}
...
AFX_STATIC void AFXAPI _AfxReleaseParkingWindow()
{
	...
	if (pThreadState->m_pWndPark != NULL)
	{
        ...
        _callback_DefWindowProc = _imp_DefWindowProc;
		::SetWindowLongPtr(hWnd, GWLP_WNDPROC, (INT_PTR)_callback_DefWindowProc);
		::DestroyWindow(hWnd);
...
{	
	...
	OLECHAR szGuid[40];
	if (::StringFromGUID2(*pGuid, szGuid, 40) == 0)
	{
		dc << "(NULL)";
		return;
	}
