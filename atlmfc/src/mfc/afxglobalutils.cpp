...
CDockingManager* CGlobalUtils::GetDockingManager(CWnd* pWnd)
{
	...
	{
		if (pWnd->m_hWnd == AfxGetMainWnd()->m_hWnd)
		{
			m_bDialogApp = TRUE;
		}
	}
...
BOOL CGlobalUtils::CyFromString(CY& cy, LPCTSTR psz)
{
	...
	VERIFY_STACK_FOR_T2OLE_RETURN(psz, FALSE);
#pragma warning(suppress: 6255)	// VERIFY_STACK_FOR_T2OLE_RETURN ensures enough stack space for below T2COLE conversion
...
HICON CGlobalUtils::GetWndIcon(CWnd* pWnd)
{
	...
	if (hIcon == NULL)
	{
		hIcon = (HICON)(LONG_PTR)::GetClassLongPtr(pWnd->m_hWnd, GCLP_HICONSM);
	}

	if (hIcon == NULL)
	{
		hIcon = (HICON)(LONG_PTR)::GetClassLongPtr(pWnd->m_hWnd, GCLP_HICON);
	}
