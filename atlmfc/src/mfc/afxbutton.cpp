...
AFX_EXPORT BOOL CMFCButton::m_bWinXPTheme = FALSE;
AFX_EXPORT BOOL CMFCButton::m_bWinXPThemeWasChecked = FALSE;
...
void CMFCButton::OnMouseMove(UINT nFlags, CPoint point)
{
		...
		(m_bWinXPTheme && !m_bDontUseWinXPTheme))
	{
		...
		if (rectClient.PtInRect(point) && WindowFromPoint(ptScreen)->m_hWnd == m_hWnd)
		{
			m_bHover = TRUE;
	...
	if (pMsg->message == WM_KEYDOWN && m_bAutoCheck && GetParent() != NULL)
	{
		...
		case VK_SPACE:
			if (m_bCheckButton)
			{
				...
				::SendMessage(pWndParent->m_hWnd, WM_COMMAND, MAKELONG(::GetWindowLong(m_hWnd, GWL_ID), BN_CLICKED), (LPARAM) m_hWnd);
...
BOOL CMFCButton::CheckNextPrevRadioButton(BOOL bNext)
{
	...
	if (pBtn != NULL && pBtn != this && !pBtn->m_bChecked)
	{
		...
		::SendMessage(pBtn->GetParent()->m_hWnd, WM_COMMAND, MAKELONG(::GetWindowLong(pBtn->m_hWnd, GWL_ID), BN_CLICKED), (LPARAM) pBtn->m_hWnd);
		return TRUE;
	}
