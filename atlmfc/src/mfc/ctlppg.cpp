...
void COlePropertyPage::OnSetPageSite()
{
	...
	{
		ENSURE(strCaption.LoadString(AFX_IDS_PROPPAGE_UNKNOWN));
	}
	...
	LPCRECT pRect, BOOL)
{
	...
	if (bSuccess)
	{
		...
		pThis->m_nControls = 0;
		::EnumChildWindows(pThis->m_hWnd, (WNDENUMPROC) COlePropertyPage::EnumChildProc, (LPARAM) pThis);

		if (pThis->m_nControls > 0)
			pThis->m_pStatus = new AFX_PPFIELDSTATUS [UINT(pThis->m_nControls)];

		pThis->m_nControls = 0;
		EnumChildWindows(pThis->m_hWnd, (WNDENUMPROC) COlePropertyPage::EnumControls, (LPARAM) pThis);
