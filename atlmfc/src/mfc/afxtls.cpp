...
int CThreadSlotData::AllocSlot()
{
    ...
    if (nSlot < nAlloc && m_pSlotData == NULL)
    {
...
void CThreadSlotData::SetValue(int nSlot, void* pValue)
{
	...
	if (pData == NULL || nSlot >= pData->nCount && pValue != NULL)
	{
		...
		void** ppvTemp;
		const auto nSize = ::ATL::AtlMultiplyThrow(static_cast<UINT>(m_nMax), static_cast<UINT>(sizeof(LPVOID)));
		if (pData->pData == NULL)
			ppvTemp = (void**)LocalAlloc(LMEM_FIXED, static_cast<UINT>(nSize));
		else
			ppvTemp = (void**)LocalReAlloc(pData->pData, static_cast<UINT>(nSize), LMEM_MOVEABLE);
		if (ppvTemp == NULL)
		{
			LeaveCriticalSection(&m_sect);
			AfxThrowMemoryException();
...
void CThreadSlotData::DeleteValues(CThreadData* pData, HINSTANCE hInst)
{
	...
	if (bDelete)
	{
		// remove from main list and free it
		EnterCriticalSection(&m_sect);
		m_list.Remove(pData);
		LeaveCriticalSection(&m_sect);
...
void CThreadSlotData::DeleteValues(HINSTANCE hInst, BOOL bEvery)
{
	EnterCriticalSection(&m_sect);
	if (!bEvery)
	{
...
void AFXAPI AfxTermLocalData(HINSTANCE hInst, BOOL bEvery)
{
	if (_afxThreadData != NULL)
		_afxThreadData->DeleteValues(hInst, bEvery);
}
