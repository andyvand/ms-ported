...
#if defined(_AFX_DLL) && defined(_MFC_BLD)
DECLSPEC_IMPORT HIMAGELIST WINAPI ImageList_Read(LPSTREAM pstm);
DECLSPEC_IMPORT WINBOOL WINAPI ImageList_Write(HIMAGELIST himl,LPSTREAM pstm);
#else
HIMAGELIST WINAPI ImageList_Read(LPSTREAM pstm);
WINBOOL WINAPI ImageList_Write(HIMAGELIST himl,LPSTREAM pstm);
#endif
...
_Use_decl_annotations_ BOOL CDragListBox::BeginDrag(CPoint pt)
{
	m_nLast = -1;
...
int CStatusBarCtrl::GetText(_Inout_ LPTSTR lpszText, int nPane, int* pType) const
{
	ASSERT(::IsWindow(m_hWnd));
	ASSERT(nPane < 256);
...
int CListCtrl::HitTest(CPoint pt, _Out_opt_ UINT* pFlags) const
{
	ASSERT(::IsWindow(m_hWnd));
	LVHITTESTINFO hti;
	hti.pt = pt;
