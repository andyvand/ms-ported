...
void CMDIClientAreaWnd::EnableMDITabs(BOOL bEnable, const CMDITabInfo& params)
{
	...
	if (m_hWnd != NULL && GetParentFrame() != NULL)
	{
...
void CMDIClientAreaWnd::OnMDIDestroy(CWnd* pWndMDIChild)
{
    ...
    AFXASSUME(pParentFrame != nullptr);
	...
	if (pMDIChild != NULL)
	{
		...
		if (pTabWnd != NULL)
		{
			...
			int iTab = pTabWnd->GetTabFromHwnd(pWndMDIChild->m_hWnd);
			if (iTab >= 0)
			{
	...
	if (m_wndTab.m_hWnd != NULL)
	{
		...
		int iTab = m_wndTab.GetTabFromHwnd(pWndMDIChild->m_hWnd);
		if (iTab >= 0)
		{
...
LRESULT CMDIClientAreaWnd::OnActiveTabChanged(WPARAM wp, LPARAM lp)
{
	...
	// if pNextMDIChild is NULL we insert at the end, because m_hWnd will return NULL
...
CMDIChildWndEx* CMDIClientAreaWnd::FindNextRegisteredWithTaskbarMDIChild(CMDIChildWndEx* pOrgWnd)
{
	...
	int iIndex = -1;
	CMFCTabCtrl* pTabCtrl = FindTabWndByChild(pOrgWnd->m_hWnd, iIndex);
...
CMDIChildWndEx* CMDIClientAreaWnd::FindNextRegisteredWithTaskbarMDIChild(CMFCTabCtrl* pTabCtrl, int iStartFrom)
{
	...
	for (int i = iStartFrom; i < pTabCtrl->GetTabsNum(); i++)
	{
		...
		if (pMDIChild == NULL || pMDIChild->m_hWnd == NULL)
		{
			continue;
		}
...
CMFCTabCtrl* CMDIClientAreaWnd::CreateTabGroup(CMFCTabCtrl* pWndTab)
{
	...
	if (pWndTab == NULL)
	{
		if (m_mdiTabParams.m_bReuseRemovedTabGroups && m_lstRemovedTabbedGroups.GetCount() > 0 && m_bIsMDITabbedGroup)
		{
			...
			if (pWndTab != NULL && ::IsWindow(pWndTab->m_hWnd))
			{
				bReusingTab = TRUE;
			}
		}
...
void CMDIClientAreaWnd::UpdateTabs(BOOL bSetActiveTabVisible/* = FALSE*/)
{
	...
	if (m_wndTab.m_hWnd == NULL || !m_bTabIsVisible)
	{
		return;
	}
	...
	while (pWndChild != NULL)
	{
		...
		int iTabIndex = m_wndTab.GetTabFromHwnd(pWndChild->m_hWnd);
		if (iTabIndex >= 0)
		{
...
void CMDIClientAreaWnd::UpdateMDITabbedGroups(BOOL bSetActiveTabVisible)
{
	...
	while (pWndChild != NULL)
	{
		...
		if (pRelatedTabWnd == NULL && !pMDIChild->m_bToBeDestroyed && bIsShowTab)
		{
			...
			hwndActive = pWndChild->m_hWnd;

		}
		else if (pRelatedTabWnd != NULL)
		{
			int iTabIndex = pRelatedTabWnd->GetTabFromHwnd(pWndChild->m_hWnd);
			if (iTabIndex >= 0)
			{
				// Tab is already exist, update it:
				if (pWndChild->GetStyle() & WS_VISIBLE)
		...
		if (pImageList != NULL)
		{
			...
			if (m_mdiTabParams.m_bTabIcons)
			{
				...
				if (!bRemoved)
				{
					int iTabIndex = pRelatedTabWnd->GetTabFromHwnd(pMDIChild->m_hWnd);
...
BOOL CMDIClientAreaWnd::MoveWindowToTabGroup(CMFCTabCtrl* pTabWndFrom, CMFCTabCtrl* pTabWndTo, int nIdxFrom)
{
	...
	HWND hwndFrom = pTabWndFrom->m_hWnd;
	HWND hwndTo	  = pTabWndTo->m_hWnd;
...
void CMDIClientAreaWnd::SerializeOpenChildren(CArchive& ar)
{
	if (ar.IsStoring())
	{
		...
		for (POSITION pos = lstWindows.GetHeadPosition(); pos != NULL;)
		{
			...
			BOOL bActive = (pMDIChild->m_hWnd == hwndActive);
			ar << bActive;
		...
		if (m_bIsMDITabbedGroup)
		{
			...
			for (POSITION pos = m_lstTabbedGroups.GetHeadPosition(); pos != NULL;)
			{
				...
				if (pNextWnd->IsActiveInMDITabGroup())
				{
					...
					PostMessage(WM_MDIACTIVATE, (WPARAM) pWnd->m_hWnd);
				}
			}
		}
...
void CMDIClientAreaWnd::OnMDINext(CWnd* pWndMDIChild, BOOL bIsPrev)
{
	...
	if (pWnd->m_hWnd != pWndMDIChild->m_hWnd)
	{
		SetActiveTab(pWnd->m_hWnd);
	}
}
