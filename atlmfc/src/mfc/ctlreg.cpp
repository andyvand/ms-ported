	...
	for (int i = 0; i < cTypeInfo; i++)
	{
			...
			(tk == TKIND_DISPATCH || tk == TKIND_INTERFACE))
		{
			...
			if (SUCCEEDED(pTypeLib->GetTypeInfo(i, &pTypeInfo)))
			{
				...
				{
#if defined(_UNICODE)
					int cchGuid = StringFromGUID2(pTypeAttr->guid, pszGuid, GUID_CCH);
#else
					WCHAR wszGuid[GUID_CCH];
					int cchGuid = StringFromGUID2(pTypeAttr->guid, wszGuid, GUID_CCH);
					_wcstombsz(pszGuid, wszGuid, cchGuid);
#endif
			...
			ERROR_SUCCESS)
		{
				...
				ERROR_SUCCESS)
			{
					...
					ERROR_SUCCESS)
				{
					...
					error = _AfxRecursiveRegDeleteKey(hKeyLocale, const_cast<LPTSTR>(TYPELIB_PLATFORM_1));
