...
LPTSTR CInternetFile::ReadString(_Out_writes_z_(nMax) LPTSTR pstr, _In_ UINT nMax)
{
	...
	if (m_pbReadBuffer == NULL)
	{
		if (!SetReadBufferSize(4096))   // arbitrary but reasonable
			return NULL;
		_Analysis_assume_(m_pbReadBuffer != NULL);
		if (!InternetReadFile(m_hFile, m_pbReadBuffer, m_nReadBufferSize,
				&dwRead))
			AfxThrowInternetException(m_dwContext);
		m_nReadBufferBytes = dwRead;
	...
	CString /*strRequestedAttributes*/, CString& strResult)
{
	...
	// It is suspicious that strRequestedAttributes is unused and
	// pstrResult is passed in here as the requested attributes.  Also
	// NULL is passed into the output results parameter of the function call.
	// However, because Gopher is not supported after XP and Server 2003R2
	// this warning will be suppressed rather than change the behavior.
