...
BOOL CMFCControlContainer::SubclassDlgControls()
{
	...
	if (m_pWnd->m_hWnd != NULL)
	{
		...
		while (pWndChild != NULL)
		{
			...
			::GetClassName(pWndChild->m_hWnd, lpszClassName, MAX_CLASS_NAME);
			...
			if (pWndSubclassedCtrl != NULL)
			{
				...
				pWndSubclassedCtrl->SubclassWindow(pWndChild->m_hWnd);
			}
...
void CMFCControlContainer::PreUnsubclassControl(CWnd* pControl)
{
	...
	if (pListCtrl != NULL && pListCtrl->GetHeaderCtrl().m_hWnd != NULL)
	{
