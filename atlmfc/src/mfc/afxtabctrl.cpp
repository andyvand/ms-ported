...
void CMFCTabCtrl::AdjustTabs()
{
	...
	if (m_pToolTipClose->m_hWnd != NULL)
	{
	...
	if (m_bActiveTabCloseButton && m_iActiveTab >= 0)
	{
		...
		if (m_pToolTipClose->m_hWnd != NULL)
		{
			...
			if (m_pToolTip->m_hWnd != NULL && pActiveTab != NULL)
			{
...
int CMFCTabCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (m_bScroll)
	{
		...
		if (m_bFlat)
		{
			...
			m_lstButtons.AddTail(m_btnScrollFirst.m_hWnd);
		}
		...
		m_lstButtons.AddTail(m_btnScrollLeft.m_hWnd);
		...
		m_lstButtons.AddTail(m_btnScrollRight.m_hWnd);
		...
		if (m_bFlat)
		{
			...
			m_lstButtons.AddTail(m_btnScrollLast.m_hWnd);
		}
		...
		m_lstButtons.AddTail(m_btnClose.m_hWnd);
...
BOOL CMFCTabCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_LBUTTONDBLCLK && pMsg->hwnd == m_btnClose.m_hWnd)
	{
		return TRUE;
	}
...
void CMFCTabCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (pScrollBar->m_hWnd == m_wndScrollWnd.m_hWnd)
	{
...
void CMFCTabCtrl::RecalcLayout()
{
	...
	if (m_bAutoSizeWindow)
	{
		for (int i = 0; i < m_iTabsNum; i ++)
		{
			...
			if (pTab->m_bVisible && pTab->m_pWnd->m_hWnd != NULL)
			{
				CRect rectWndArea = m_rectWndArea;
				CWnd* pParent = pTab->m_pWnd->GetParent();

				if (pParent->m_hWnd != NULL && pParent->m_hWnd != m_hWnd)
				{
...
BOOL CMFCTabCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (pNMHDR->code == TTN_SHOW)
	{
		...
		if (m_pToolTipClose->m_hWnd != NULL && pNMHDR->hwndFrom == m_pToolTipClose->m_hWnd)
		{
...
BOOL CMFCTabCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if ((HWND)lParam == m_btnScrollLeft.m_hWnd)
	{
		bScrollTabs = TRUE;
	...
	else if ((HWND)lParam == m_btnScrollRight.m_hWnd)
	{
		if (m_bTabDocumentsMenu)
		{
			CRect rectButton;
	...
	else if ((HWND)lParam == m_btnScrollFirst.m_hWnd)
	{
		bScrollTabs = TRUE;
		m_nTabsHorzOffset = 0;
	}
	else if ((HWND)lParam == m_btnScrollLast.m_hWnd)
	{
		bScrollTabs = TRUE;
		m_nTabsHorzOffset = m_nTabsHorzOffsetMax;
	}
	else if ((HWND)lParam == m_btnClose.m_hWnd)
	{
		CWnd* pWndActive = GetActiveWnd();
		if (pWndActive != NULL)
		{
...
void CMFCTabCtrl::ReposButtons(CPoint pt, CSize sizeButton, BOOL bHide, int nButtonMargin)
{
	...
	for (POSITION pos = m_lstButtons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (bHide ||(!bCloseBtn && hWndButton == m_btnClose.m_hWnd) || (m_bTabDocumentsMenu && bIsFirst))
		{
...
void CMFCTabCtrl::UpdateScrollButtonsState()
{
	...
	if (m_hWnd == NULL || !m_bScroll || m_bFlat)
	{
		return;
	}
...
void CMFCTabCtrl::OnShowTabDocumentsMenu(CPoint point)
{
	...
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		if (pTab->m_pWnd->m_hWnd != NULL)
		{
			HICON hIcon = pTab->m_pWnd->GetIcon(FALSE);
			if (hIcon == NULL)
			{
				hIcon = (HICON)(LONG_PTR) GetClassLongPtr(pTab->m_pWnd->m_hWnd, GCLP_HICONSM);
			}
...
void CMFCTabCtrl::ActivateMDITab(int nTab)
{
	...
	if (pActiveWnd != NULL)
	{
		...
		GetParent()->SendMessage(WM_MDIACTIVATE, (WPARAM) pActiveWnd->m_hWnd);
		pActiveWnd->SetFocus();
	}
}
