...
BOOL CMFCReBarState::LoadRebarStateProc(HWND hwnd, LPARAM lParam)
{
	...
	CString strRebar;
	strRebar.Format(AFX_REBAR_FORMAT_KEY, GetWindowLong(rc.m_hWnd, GWL_ID));
...
void __stdcall CMFCReBarState::LoadState(CString& strRegKey, CFrameWnd* pFrame)
{
	ASSERT_VALID(pFrame);
	EnumChildWindows(pFrame->m_hWnd, LoadRebarStateProc, (LPARAM)(LPCTSTR)strRegKey);
}

void __stdcall CMFCReBarState::SaveState(CString& strRegKey, CFrameWnd* pFrame)
{
	ASSERT_VALID(pFrame);
	EnumChildWindows(pFrame->m_hWnd, SaveRebarStateProc, (LPARAM)(LPCTSTR)strRegKey);
}
