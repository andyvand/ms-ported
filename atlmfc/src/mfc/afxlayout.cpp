...
{
	if (m_pHostWnd->m_hWnd == NULL)
	{
		ASSERT(FALSE);
		return FALSE;
	}

	return AddItem(m_pHostWnd->GetDlgItem(nID)->m_hWnd, moveSettings, sizeSettings);
}

BOOL CMFCDynamicLayout::AddItem(HWND hWnd, MoveSettings moveSettings, SizeSettings sizeSettings)
{
	if (hWnd == NULL || !::IsWindow(hWnd) || !::IsChild(m_pHostWnd->m_hWnd, hWnd))
	{
		ASSERT(FALSE);
		return FALSE;
	}
...
{
	if (pHostWnd->m_hWnd == NULL || m_listCtrls.IsEmpty())
	{
		return FALSE;
	}
	...
	while (pChild != NULL && pos != NULL)
	{
		...
		{
			pLayout->AddItem(pChild->m_hWnd, item.m_moveSettings, item.m_sizeSettings);
			nCount++;
		}
...
{
	if (pHostWnd->m_hWnd == NULL || !::IsWindow(pHostWnd->m_hWnd) || lpResource == NULL)
	{
		return FALSE;
	}
