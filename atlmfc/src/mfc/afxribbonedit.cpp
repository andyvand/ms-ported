...
{
	if (m_pEdit != NULL && GetBuddy()->m_hWnd != NULL)
	{
		CString str;
...
void CMFCRibbonEdit::Redraw()
{
	...
	if (m_pWndEdit->m_hWnd != NULL && m_pWndEdit->IsWindowVisible())
	{
		m_pWndEdit->RedrawWindow();
	}

	if (m_pWndSpin->m_hWnd != NULL && m_pWndSpin->IsWindowVisible())
	{
...
void CMFCRibbonEdit::OnRTLChanged(BOOL bIsRTL)
{
	...
	if (!m_rect.IsRectEmpty() && m_pWndEdit->m_hWnd != NULL && m_pWndEdit->IsWindowVisible())
	{
...
BOOL CMFCRibbonRichEditCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_LBUTTONDOWN && m_edit.m_bIsEditFocused && !m_edit.m_bIsFocused && pMsg->hwnd != m_hWnd)
	{
	...
	if (pMsg->message == WM_KEYDOWN && !m_edit.IsDisabled())
	{
		...
		case VK_NEXT:
			...
			{
				::SendMessage( CMFCPopupMenu::GetActiveMenu()->m_hWnd, WM_KEYDOWN, pMsg->wParam, pMsg->lParam);
				return TRUE;
			}
			break;
		...
		case VK_RETURN:
			if (!m_edit.IsDroppedDown())
			{
				...
				if (m_edit.m_pParentMenu != NULL && m_edit.m_pParentMenu->GetParent()->m_hWnd == CMFCPopupMenu::GetActiveMenu()->m_hWnd)
				{
