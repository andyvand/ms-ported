...
{
	if (m_pMenuDlgImpl != NULL && m_pMenuDlgImpl->m_Dlg.m_hWnd == m_Dlg.m_hWnd)
	{
		m_pMenuDlgImpl = NULL;
	}
}
...
void CDialogImpl::OnActivate(UINT nState, CWnd* pWndOther)
{
	...
	if (pWndActive != NULL)
	{
		BOOL bStayActive = (pWndActive->m_hWnd == m_Dlg.m_hWnd || pWndActive->SendMessage(WM_FLOATSTATUS, FS_SYNCACTIVE));
