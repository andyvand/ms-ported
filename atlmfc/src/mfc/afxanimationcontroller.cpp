...
void CAnimationVariable::EnableValueChangedEvent (CAnimationController* pController, BOOL bEnable)
{
	...
	if (!bEnable)
	{
		if (FAILED(m_variable->SetVariableChangeHandler(NULL)))
		{
			TRACE("Failed to disable Animation Variable Changed Event Handler.\n");
		}
		return;
	}
	...
	{
		TRACE("Failed to create Animation Variable Changed Event Handler.\n");
		return;
	}

	if (FAILED(m_variable->SetVariableChangeHandler(pValueChangeHandler)))
	{
		TRACE("Failed to set Animation Variable Changed Event Handler.\n");
	}
}
...
{
	if (newStatus == UI_ANIMATION_MANAGER_BUSY && m_pRelatedWnd->m_hWnd != NULL && ::IsWindow(m_pRelatedWnd->m_hWnd))
	{
...
{
	if (m_pRelatedWnd->m_hWnd != NULL && ::IsWindow(m_pRelatedWnd->m_hWnd))
	{
