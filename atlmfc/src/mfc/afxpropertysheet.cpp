...
CMFCPropertySheetCategoryInfo::~CMFCPropertySheetCategoryInfo()
{
	...
	if (m_propSheet.m_hWnd != NULL)
	{
		...
		if (m_propSheet.m_wndTree.m_hWnd != NULL && m_hTreeItem != NULL)
		{
...
void CMFCPropertySheet::AddPage(CPropertyPage* pPage)
{
	...
	if (m_hWnd == NULL || m_look == PropSheetLook_Tabs)
	{
		return;
	}
...
BOOL CMFCPropertySheet::IsLeftNavigationPane(HWND hWnd) const
{
	...
	return m_wndList.m_hWnd == hWnd || m_wndOutlookBar.m_hWnd == hWnd || m_wndTree.m_hWnd == hWnd;
}

BOOL CMFCPropertySheet::CanAddPageToDynamicLayout() const
{
	return m_wndTab.m_hWnd == NULL;
}
