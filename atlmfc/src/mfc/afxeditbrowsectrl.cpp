...
{
	CString strError;
	ENSURE(strError.LoadString(AFX_IDP_INVALID_FILENAME));
...
void CMFCEditBrowseCtrl::OnAfterUpdate()
{
	...
	GetOwner()->PostMessage(EN_CHANGE, GetDlgCtrlID(), (LPARAM) m_hWnd);
	GetOwner()->PostMessage(EN_UPDATE, GetDlgCtrlID(), (LPARAM) m_hWnd);
}
