...
CEnumFormatEtc::~CEnumFormatEtc()
{
	if (m_pClonedFrom == NULL)
	{
		...
		#pragma warning(push)
		#pragma warning(disable:6001)	// Using uninitialized memory '*lpFormatEtc.ptd'
										// Note that lpFormatEtc is initialized from m_pvEnum 
										// from the CEnumArray array class which uses m_nSize 
										// (used below for the for loop bounds) to track how many
										// elements it contains, so lpFormatEtc[nIndex].ptd should
										// be valid
...
void CEnumFormatEtc::AddFormat(const FORMATETC* lpFormatEtc)
{
	...
	pFormat->ptd = lpFormatEtc->ptd;
		// Note: ownership of lpFormatEtc->ptd is transferred with this call.
