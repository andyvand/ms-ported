...
BOOL CProperty::Set(const  LPVOID pVal)
{
	...
	{
		_Analysis_assume_(m_pValue != NULL);
...
BOOL CProperty::ReadFromStream(IStream* pIStream)
{
	...
	if (FAILED(pIStream->Read((LPVOID)&m_dwType, sizeof(m_dwType), &cb)) ||
		cb != sizeof(m_dwType))
		return FALSE;
	...
	if (FAILED(pIStream->Clone(&pIStrItem)))
		return FALSE;
	...
	if (m_dwType & VT_VECTOR)
	{
		...
		// elements
		if (FAILED(pIStrItem->Read((LPVOID)&nReps, sizeof(nReps), &cb)) ||
			cb != sizeof(nReps))
			return FALSE;
		cbValue += cb;
		dwType &= ~VT_VECTOR;
	...
	// Since a value can be made up of a vector (VT_VECTOR) of
	// items, we first seek through the value, picking out
	// each item, getting its size.  We use a cloned
	...
	while (iReps--)
	{
		switch (dwType)
		{
			...
			case VT_CF:             // Clipboard format
				// Read the DWORD that gives us the size, making
				// sure we increment cbValue.
				if (FAILED(pIStrItem->Read((LPVOID)&cbItem, sizeof(cbItem), &cb)) ||
					cb != sizeof(cbItem))
					return FALSE;
				li.QuadPart = -(LONG)cb;
	...
	switch (dwType)
	{
		...
		case VT_STORED_PROPSET: // Storage contains a propset
			...
			if (FAILED(pIStream->Read(pTmp, cbValue, &cb)))
				return FALSE;
		...
		default:
			...
			if (FAILED(pIStream->Read(m_pValue, cbValue, &cb)) ||
				cb != cbValue)
				return FALSE;
#ifdef _UNICODE
			break;
...
BOOL CPropertySection::WriteToStream(IStream* pIStream)
{
	...
	//
	if (FAILED(pIStream->Clone(&pIStrPIDO)))
		return FALSE;
	...
	LARGE_INTEGER liPropSet)
{
	...
	if (FAILED(pIStream->Read((LPVOID)&m_SH, sizeof(m_SH), &cb)) ||
		cb != sizeof(m_SH))
		return FALSE;
	...
	while (cProperties--)
	{
		if (FAILED(pIStrPIDO->Read((LPVOID)&pido, sizeof(pido), &cb)) ||
			cb != sizeof(pido))
		{
			pIStrPIDO->Release();
			return FALSE;
...
BOOL CPropertySection::ReadNameDictFromStream(IStream* pIStream)
{
	...
	if (FAILED(pIStream->Read((LPVOID)&cProperties, sizeof(cProperties), &cb)) ||
		sizeof(cProperties) != cb)
	{
...
BOOL CPropertySet::WriteToStream(IStream* pIStream)
{
	...
	//
	if (FAILED(pIStream->Clone(&pIStrFIDO)))
		return FALSE;
...
BOOL CPropertySet::ReadFromStream(IStream* pIStream)
{
	...
	if (FAILED(pIStream->Read((LPVOID)&m_PH, sizeof(m_PH), &cb)) ||
		cb != sizeof(m_PH))
		return FALSE;
	...
	while (cSections--)
	{
		if (FAILED(pIStrFIDO->Read((LPVOID)&fido, sizeof(fido), &cb)) ||
			cb != sizeof(fido))
		{
			pIStrFIDO->Release();
			return FALSE;
