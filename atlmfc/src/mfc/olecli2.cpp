...
void COleFrameHook::OnPaletteChanged(CWnd* pFocusWnd)
{
	...
	if (pWnd != NULL)
		pWnd->SendMessage(WM_PALETTECHANGED, (WPARAM)pFocusWnd->m_hWnd);
}
...
BOOL COleClientItem::OnGetWindowContext(CFrameWnd** ppMainFrame, CFrameWnd** ppDocFrame, LPOLEINPLACEFRAMEINFO pFrameInfo)
{
	...
	{
		#pragma warning(push)
		#pragma warning(disable:6214) // Cast between semantically different integer types: HRESULT to a Boolean type
									  // This returns a nonzero value indicating success (according to the documentation). 
									  // We could avoid suppressing this warning by returning TRUE here, but given that the 
									  // returned value is so specific (E_POINTER) and this is a publicly-available function, 
									  // I'm erring on the side of caution and leaving the return's value unchanged. 
		return E_POINTER;
		#pragma warning(pop)
	}
	...
	LPCRECT lpRectWidths)
{
		...
		CFrameWnd::borderSet, (LPRECT)lpRectWidths))
	{
		...
		// doesn't call back into the object and tell it to resize its borders
