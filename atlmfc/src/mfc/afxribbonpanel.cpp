...
CMFCRibbonBaseElement* CMFCRibbonPanel::MouseButtonDown(CPoint point)
{
	...
	if (m_pHighlighted != NULL)
	{
		...
		HWND hwndMenu = m_pParentMenuBar->m_hWnd;
...
void CMFCRibbonPanel::MouseButtonUp(CPoint point)
{
	...
	if (m_pHighlighted != NULL)
	{
		...
		HWND hwndParent = GetParentWnd()->m_hWnd;
...
BOOL CMFCRibbonPanel::SetElementMenu(UINT uiCmdID, HMENU hMenu, BOOL bIsDefaultCommand, BOOL bRightAlign)
{
	ASSERT_VALID(this);
	...
	ASSERT_VALID(pButton);
	pButton->SetMenu(hMenu, bIsDefaultCommand, bRightAlign);

	return TRUE;
}

BOOL CMFCRibbonPanel::SetElementMenu(UINT uiCmdID, UINT uiMenuResID, BOOL bIsDefaultCommand, BOOL bRightAlign)
{
	ASSERT_VALID(this);
	...
	ASSERT_VALID(pButton);
	pButton->SetMenu(uiMenuResID, bIsDefaultCommand, bRightAlign);
...
BOOL CMFCRibbonPanel::OnKey(UINT nChar)
{
	...
	case VK_TAB:
		if (m_pHighlighted != NULL)
		{
			if (m_pPaletteButton != NULL && (nChar == VK_DOWN || nChar == VK_UP) && m_pParentMenuBar->m_hWnd != NULL && m_pScrollBar->m_hWnd != NULL)
			{
	...
	if (pNewHighlighted != NULL)
	{
		...
		if (m_pHighlighted != pNewHighlighted)
		{
			...
			if (GetGlobalData()->IsAccessibilitySupport() && m_pParentMenuBar != NULL && IsMenuMode())
			{
				...
				::NotifyWinEvent(EVENT_OBJECT_FOCUS, m_pParentMenuBar->m_hWnd, OBJID_CLIENT, (LONG)GetIndex(pNewHighlighted) + 1);
			}
	...
	if (bInvokeCommand && m_pHighlighted != NULL)
	{
		...
		if (pButton != NULL)
		{
			if (pButton->HasMenu())
			{
				HWND hWndParent = GetParentMenuBar()->m_hWnd;
...
HRESULT CMFCRibbonPanel::accHitTest(long xLeft, long yTop, VARIANT *pvarChild)
{
    ...
    if (pParentRibbonBar->m_hWnd == NULL)
    {
        return S_FALSE;
    }
