...
CMFCRibbonPanelMenuBar::~CMFCRibbonPanelMenuBar()
{
	...
	if (m_bHasKeyTips)
	{
		...
		if (pMenu != NULL && CWnd::FromHandlePermanent(pMenu->m_hWnd) != NULL && pMenu->IsWindowVisible())
		{
	...
	else if (nIDEvent == AFX_TIMER_ID_ACCELERATOR_NOTIFY_EVENT)
	{
		...
		if (nIndex != -1)
		{
			if (m_iAccHotItem == nIndex && m_iAccHotItem != -1)
			{
				...
				{
					::NotifyWinEvent(EVENT_OBJECT_FOCUS, m_hWnd, OBJID_CLIENT, nIndex + 1);
				}
			}
		}
	}
...
void CMFCRibbonPanelMenuBar::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (m_pPanel == NULL || pScrollBar->m_hWnd != m_wndScrollBarVert.m_hWnd || m_pPanel->m_pPaletteButton == NULL)
	{
