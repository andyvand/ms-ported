...
_AfxHandleActivate(CWnd* pWnd, WPARAM nState, CWnd* pWndOther)
{
	...
	if (!(pWnd->GetStyle() & WS_CHILD))
	{
		...
		if (pTopLevel && (pWndOther == NULL || !::IsWindow(pWndOther->m_hWnd) || pTopLevel != pWndOther->GetTopLevelParent()))
		{
			...
			HWND hWnd2[2];
			hWnd2[0] = pWnd->m_hWnd;
			hWnd2[1] = (pWndOther == NULL) ? NULL : pWndOther->m_hWnd;
			// send it...
	...
	TRY
	{
		...
		case WM_ACTIVATE:
			_AfxHandleActivate(CWnd::FromHandle(hWnd), wParam,
				lParam == 0 ? NULL : CWnd::FromHandle((HWND)lParam));
			break;
	...
	if (pWndInit != NULL || (!(lpcs->style & WS_CHILD) && !bContextIsDLL))
	{
		...
		else
		{
			...
			if (bSubclass)
			{
				...
				{
					SetProp(hWnd, _afxOldWndProc, (HANDLE)oldWndProc);
		...
		LPVOID lpParam /* = NULL */)
{
	...
	return CreateEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle,
		rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
                    pParentWnd == NULL ? NULL : pParentWnd->m_hWnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(nID)), lpParam);
}
	...
	CCreateContext* pContext)
{
	...
	return CreateEx(0, lpszClassName, lpszWindowName,
		...
		rect.right - rect.left, rect.bottom - rect.top,
                    pParentWnd == NULL ? NULL : pParentWnd->m_hWnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(nID)), (LPVOID)pContext);
}
...
BOOL CWnd::RegisterTouchWindow(BOOL bRegister, ULONG ulFlags)
{
	...
	if (!bRegister)
	{
		return (*pfUnregister)(m_hWnd);
	}

	m_bIsTouchWindowRegistered = (*pfRegister)(m_hWnd, ulFlags);
...
BOOL CWnd::SetGestureConfig(CGestureConfig* pConfig)
{
	...
	return (*pfSetGestureConfig)(m_hWnd, 0, cIDs, pConfigs, sizeof(GESTURECONFIG));
}
...
BOOL CWnd::GetGestureConfig(CGestureConfig* pConfig)
{
	...
	return (*pfGetGestureConfig)(m_hWnd, 0, 0, &cIDs, pConfigs, sizeof(GESTURECONFIG));
}
...
LRESULT CWnd::OnGesture(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (!(*pfGetGestureInfo)((HGESTUREINFO)lParam, m_pCurrentGestureInfo) || m_pCurrentGestureInfo->hwndTarget != m_hWnd)
	{
		ZeroMemory(m_pCurrentGestureInfo, sizeof(GESTUREINFO));
		return Default ();
	}
...
BOOL CWnd::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (message == WM_ACTIVATE)
		_AfxHandleActivate(this, wParam, lParam == 0 ? NULL : CWnd::FromHandle((HWND)lParam));
...
CWnd* CWnd::GetTopLevelParent() const
{
	...
	return hWndParent == NULL ? NULL : CWnd::FromHandle(hWndParent);
}
...
CWnd* CWnd::GetTopLevelOwner() const
{
	...
	return hWndOwner == NULL ? NULL : CWnd::FromHandle(hWndOwner);
}
...
CWnd* PASCAL CWnd::GetSafeOwner(CWnd* pParent, HWND* pWndTop)
{
	...
	HWND hWnd = GetSafeOwner_(pParent->m_hWnd, pWndTop);
...
int CWnd::MessageBox(LPCTSTR lpszText, LPCTSTR lpszCaption, UINT nType)
{
	...
	int nResult = ::MessageBox(m_hWnd, lpszText, lpszCaption, nType);
	return nResult;
}
...
void CWnd::CenterWindow(CWnd* pAlternateOwner)
{
	...
	HWND hWndCenter = pAlternateOwner == NULL ? NULL : pAlternateOwner->m_hWnd;
	...
	if (!(dwStyle & WS_CHILD))
	{
		...
		if (hWndCenter == NULL)
		{
			HWND hwDefault = AfxGetMainWnd()->m_hWnd;
...
void CWnd::EnableD2DSupport(BOOL bEnable, BOOL bUseDCRenderTarget)
{
	if (bEnable)
	{
		...
		{
			m_pRenderTarget = new CHwndRenderTarget(m_hWnd);
		}
		ASSERT_VALID(m_pRenderTarget);
	}
	else
...
BOOL CWnd::DoD2DPaint()
{
	...
	if (pHwndRenderTarget != NULL)
	{
		...
		{
			pHwndRenderTarget->Create(m_hWnd);
			if (!pHwndRenderTarget->IsValid())
			{
				return FALSE;
			}
		...
		{
			pHwndRenderTarget->ReCreate(m_hWnd);
	...
	dcPaint.Attach(::BeginPaint(m_hWnd, &ps));
...
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd) || lpszResourceName == NULL)
	{
		return FALSE;
	}
