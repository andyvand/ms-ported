...
AFX_EXPORT BOOL CBasePane::m_bSetTooltipTopmost = TRUE;
...
HDWP CBasePane::MoveWindow(const CRect& rect, BOOL bRepaint, HDWP hdwp)
{
	...
	if (hdwp != NULL)
	{
		...
		return DeferWindowPos(hdwp, m_hWnd, NULL, rect.left, rect.top, rect.Width(),
			rect.Height(), uFlags);
...
HDWP CBasePane::SetWindowPos(const CWnd* pWndInsertAfter, int x, int y, int cx, int cy, UINT nFlags, HDWP hdwp)
{
	...
	HDWP hdwpNew = DeferWindowPos(hdwp, m_hWnd, NULL, x, y, cx, cy, nFlags);
	if (hdwpNew == NULL)
	{
...
void CBasePane::ShowPane(BOOL bShow, BOOL bDelay, BOOL bActivate)
{
	...
	else if (IsTabbed())
	{
		...
		{
			int iTab = pTabParent->GetTabFromHwnd(m_hWnd);
...
{
	if (m_pDockSite == NULL && GetParent()->IsKindOf(RUNTIME_CLASS(CDialog)) && GetParent()->m_hWnd == AfxGetMainWnd()->m_hWnd)
	{
...
void CBasePane::OnPaneContextMenu(CWnd* pParentFrame, CPoint point)
{
	...
	if (pParentFrame->SendMessage(AFX_WM_TOOLBARMENU, (WPARAM) m_hWnd, MAKELPARAM(point.x, point.y)) == 0)
	{
		return;
	}
