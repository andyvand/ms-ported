...
CArchive& AFXAPI operator>>(CArchive& ar, CComBSTR& string)
{
	...
	if( length > 0 )
	{
		...
		if(SysReAllocStringLen( &temp, NULL, length ))
		{
...
#pragma warning(pop)
#pragma warning(push)
#pragma warning(disable:6385) //Reading invalid data from 'piLBounds'
// it should be ok to disable warning because nDimensions has already been validated
   // Dump the bounds
...
#pragma warning(pop)
   ...
   if( dc.GetDepth() > 0 )
   {
	  ...
	  {
// it should be ok to disable warning because nDimensions has already been validated
#pragma warning(suppress:6386) //Buffer overrun while writing to 'piIndices'
	  ...
	  while( piIndices[0] <= piUBounds[0] )
	  {
		 ...
		 {
// it should be ok to disable warning because nDimensions has already been validated
#pragma warning(suppress:6385) //Reading invalid data from 'piIndices'
...
void AFXAPI DDX_Text(CDataExchange* pDX, int nIDC, GUID& value)
{
   ...
   pDX->m_pDlgWnd->GetDlgItem(nIDC, &hWndCtrl);
	...
	else
	{
		...
		int result = StringFromGUID2(value, szGUID, 64);
		AFXASSUME(result);
