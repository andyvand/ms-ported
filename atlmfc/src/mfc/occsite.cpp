...
DEFINE_GUID(IID_ICursor,0x9f6aa700, 0xd188, 0x11cd, 0xad, 0x48, 0x0, 0xaa, 0x0, 0x3c, 0x9c, 0xb6);
DEFINE_GUID(IID_INotifyDBEvents,0xdb526cc0, 0xd188, 0x11cd, 0xad, 0x48, 0x0, 0xaa, 0x0, 0x3c, 0x9c, 0xb6);
DEFINE_GUID(IID_ICursorMove,0xacff0690, 0xd188, 0x11cd, 0xad, 0x48, 0x0, 0xaa, 0x0, 0x3c, 0x9c, 0xb6);
DEFINE_GUID(IID_ICursorUpdateARow,0xd14216a0, 0xd188, 0x11cd, 0xad, 0x48, 0x0, 0xaa, 0x0, 0x3c, 0x9c, 0xb6);
...
#ifdef _DEBUG
	OLECHAR wszClsid[40];
	ENSURE(StringFromGUID2(clsid, wszClsid, 40) != 0);
#endif //_DEBUG
	...
	int cy, UINT nFlags)
{
	...
	return ::SetWindowPos(m_hWnd, pWndInsertAfter->m_hWnd,
		x, y, cx, cy, nFlags);
}
...
{
	METHOD_PROLOGUE_EX_(COleControlSite, OleIPSite)
	*phWnd = pThis->m_pCtrlCont->m_pWnd->m_hWnd;
	...
	LPRECT prectClip, LPOLEINPLACEFRAMEINFO pFrameInfo)
{
	...
	pFrameInfo->hwndFrame = pWndContainer->m_hWnd;
...
_Use_decl_annotations_ STDMETHODIMP COleControlSite::XAmbientProps::GetIDsOfNames(
...
HRESULT CDataSourceControl::GetMetaData()
{
	...
	for (int nCol = 0; nCol < m_nColumns; nCol++)
	{
#pragma warning (suppress: 6386)	// VS Developer Community Feedback submitted for this false positive. 
...
void CDataSourceControl::BindColumns()
{
	...
	// Determine number of bindings from m_pMetaRowData[0..m_nColumns]::m_pClientList
	m_nBindings = 0;
	INT_PTR nCol;
	...
	// Create new array for m_pColumnBindings of size m_mBinginds
	if (m_nBindings > 0)
	...
	for (nCol=0; nCol<m_nColumns; nCol++)
	{
		// Iterate through client list of m_pMetaRowData[nCol]
		// There must m_pClientList->GetCount() clients, as used to calculate m_nBindings above.
		POSITION pos = m_pMetaRowData[nCol].m_pClientList->GetHeadPosition();
		while (pos)
		{
			// Total number of client (represented by nItem) must be <= m_nBindings
			_Analysis_assume_(nItem < m_nBindings);
	...
	// Make sure the assumption for _Analysis_assume_ inside above loop holds true
	AFXASSUME(nItem == m_nBindings);
