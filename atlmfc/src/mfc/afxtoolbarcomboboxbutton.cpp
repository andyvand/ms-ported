...
SIZE CMFCToolBarComboBoxButton::OnCalculateSize(CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	...
	if (m_bFlat && m_pWndCombo->m_hWnd != NULL &&
		(m_pWndCombo->GetStyle() & WS_VISIBLE))
	{
		m_pWndCombo->ShowWindow(SW_HIDE);
	}
	...
	if (bHorz)
	{
		if (!m_bFlat && m_pWndCombo->m_hWnd != NULL && !m_bIsHidden)
		{
		...
		if (!m_bIsHidden && m_pWndEdit->m_hWnd != NULL && m_pWndCombo != NULL && (m_pWndCombo->GetStyle() & WS_VISIBLE) == 0)
		{
	...
	{
		if (m_pWndCombo->m_hWnd != NULL && (m_pWndCombo->GetStyle() & WS_VISIBLE))
		{
			m_pWndCombo->ShowWindow(SW_HIDE);
		}

		if (m_pWndEdit->m_hWnd != NULL && (m_pWndEdit->GetStyle() & WS_VISIBLE))
		{
			m_pWndEdit->ShowWindow(SW_HIDE);
		}
...
void CMFCToolBarComboBoxButton::OnChangeParentWnd(CWnd* pWndParent)
{
	...
	if (m_pWndCombo->m_hWnd != NULL)
	{
		...
		if (pWndParent != NULL && pWndParentCurr->m_hWnd == pWndParent->m_hWnd)
		{
			return;
		}
	...
	if (pWndParent == NULL || pWndParent->m_hWnd == NULL)
	{
		return;
	}
...
{
	if (m_pWndCombo->m_hWnd == NULL || m_rect.IsRectEmpty() || !m_bHorz)
	{
		m_rectCombo.SetRectEmpty();
		m_rectButton.SetRectEmpty();
		return;
			...
			if (CMFCToolBar::GetCommandButtons(m_nID, listButtons) > 0)
			{
				for (POSITION posCombo = listButtons.GetHeadPosition(); posCombo != NULL;)
				{
					...
					if (pCombo != NULL && pCombo != this)
					{
						...
						if (pCombo->m_pWndCombo->m_hWnd != NULL && pCombo->m_pWndCombo->GetParent() != NULL)
						{
...
{
	if (m_pWndCombo == NULL || m_pWndCombo->m_hWnd == NULL || !bHorz)
	{
		...
		if (!m_strEdit.IsEmpty())
		{
			...
			if (m_pWndEdit == NULL)
			{
				if (m_pWndCombo->GetStyle() &(CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE))
				{
					...
					dis.hwndItem = m_pWndCombo->m_hWnd;
...
{
	if (m_pWndCombo == NULL || m_pWndCombo->m_hWnd == NULL || !m_bHorz)
	{
		return FALSE;
	}
		...
		if ((GetKeyState(VK_MENU) >= 0) &&(GetKeyState(VK_CONTROL) >= 0) && m_combo.GetComboBox() != NULL)
		{
			...
			case VK_RETURN:
				...
				else if (m_combo.GetComboBox()->GetOwner() != NULL)
				{
					...
					m_combo.GetComboBox()->GetOwner()->PostMessage(WM_COMMAND, MAKEWPARAM(m_combo.m_nID, 0), (LPARAM) m_combo.GetComboBox()->m_hWnd);
				}
...
void CMFCToolBarComboBoxEdit::OnContextMenu(CWnd* pWnd, CPoint point)
{
	if (m_combo.m_uiMenuResID != 0)
	{
		...
		{
			::TrackPopupMenu(hPopupMenu, TPM_CENTERALIGN | TPM_LEFTBUTTON, point.x, point.y, 0, pWndParent->m_hWnd, NULL);
		}
	}
	else
	{
