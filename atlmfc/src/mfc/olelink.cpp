...
LPMONIKER COleLinkingDoc::GetMoniker(OLEGETMONIKER nAssign)
{
	...
	LPMONIKER lpMoniker;
	(void)::CreateFileMoniker(CStringW(m_strMoniker), &lpMoniker);
	return lpMoniker;
}
...
void COleLinkingDoc::Revoke()
{
	...
	{
		LPRUNNINGOBJECTTABLE lpROT = NULL;
		if (SUCCEEDED(::GetRunningObjectTable(0, &lpROT)) && lpROT != NULL)
		{
...
void COleLinkingDoc::LockExternal(BOOL bLock, BOOL bRemoveRefs)
{
	...
	(void)::CoLockObjectExternal((LPUNKNOWN)GetInterface(&IID_IUnknown),
		bLock, bRemoveRefs);
...
STDMETHODIMP COleLinkingDoc::XPersistFile::Save(LPCOLESTR lpszFileName, BOOL fRemember)
{
	...
	TRY
	{
		...
		{
			pThis->OnSaveDocument((LPCTSTR)NULL);
		}
		else
		{
