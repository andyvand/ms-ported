   ...
   if (m_pCtrlCont == NULL)
   {
	...
	if (m_pCtrlCont != NULL)
	{
		...
		while ((pWnd != NULL) && !(pWnd->m_nFlags & WF_OLECTLCONTAINER))
		{
			...
			pWnd = pWnd->GetParent();
			if (! (GetWindowLong(pWnd->m_hWnd, GWL_STYLE) & WS_CHILD))
				break;
		}
	}
	...
	VARIANT* pvarResult)
{
	...
	case DISPID_AMBIENT_SCALEUNITS:
		{
			CString str;
			ENSURE(str.LoadString(AFX_IDS_OCC_SCALEUNITS_PIXELS));
			V_VT(pvarResult) = VT_BSTR;
			V_BSTR(pvarResult) = str.AllocSysString();
		}
		return TRUE;
...
BOOL COleControlContainer::OnPaint( CDC* pDC )
{
   ...
   {
	  pSiteOrWnd = m_listSitesOrWnds.GetNext( pos );
	  if( pSiteOrWnd->m_pSite && pSiteOrWnd->m_pSite->m_bIsWindowless && !(pSiteOrWnd->m_pSite->m_dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) )
	  {
		 pViewObject = NULL;
		...
		else
		{
			if(pOccDlgInfo->m_pItemInfo[i].nId)
			{
				...
				HWND hwndSearchFrom= (hwndStart == NULL) ?
									 GetWindow(m_pWnd->m_hWnd,GW_CHILD) :
				...
				if (hwndCtrl == NULL)
				{
					hwndCtrl = ::GetDlgItem(m_pWnd->m_hWnd,pOccDlgInfo->m_pItemInfo[i].nId);
...
_Use_decl_annotations_ STDMETHODIMP COleControlContainer::XOleContainer::ParseDisplayName(
	LPBINDCTX, LPOLESTR, ULONG*, LPMONIKER*)
{
	return E_NOTIMPL;
}
	...
	LPENUMUNKNOWN* ppEnumUnknown)
{
	...
	*ppEnumUnknown = NULL;
	...
	TRY
	{
		...
		{
			auto nSize = pThis->m_listSitesOrWnds.GetCount();
			ppUnk = new LPUNKNOWN[nSize];
			POSITION pos = pThis->m_listSitesOrWnds.GetHeadPosition();
			COleControlSiteOrWnd* pSiteOrWnd;
			while (pos != NULL)
			{
				_Analysis_assume_(cObjects < nSize);
