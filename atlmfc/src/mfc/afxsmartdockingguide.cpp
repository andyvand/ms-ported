...
void CSmartDockingStandaloneGuide::Create(SDMarkerPlace nSideNo, CWnd* pwndOwner)
{
	...
	if (!bAlphaMarkers)
	{
		GetGlobalData()->SetLayeredAttrib(m_wndBmp.m_hWnd, params.m_clrTransparent, 0, LWA_COLORKEY);
	}
...
{
	if (m_wndBmp.m_hWnd == NULL || !m_wndBmp.IsWindowVisible())
	{
		return FALSE;
	}
...
#define AFX_BASE_PTS 4
	...
	if (bResult)
	{
		...
		if (!params.m_bIsAlphaMarkers && GetVMTheme() != AFX_SDT_VS2008)
		{
			GetGlobalData()->SetLayeredAttrib(m_Wnd.m_hWnd, params.m_clrTransparent, 0, LWA_COLORKEY);
		}
		else
		{
			m_Wnd.Update();
