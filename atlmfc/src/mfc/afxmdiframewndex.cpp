...
void CMDIFrameWndEx::OnClosePopupMenu(CMFCPopupMenu* pMenuPopup)
{
	if (GetGlobalData()->IsAccessibilitySupport() && pMenuPopup != NULL)
	{
		...
		{
			::NotifyWinEvent(EVENT_SYSTEM_MENUPOPUPEND, pMenuPopup->m_hWnd, OBJID_WINDOW, CHILDID_SELF);
		}
		else
		{
			::NotifyWinEvent(EVENT_SYSTEM_MENUEND, pMenuPopup->m_hWnd, OBJID_WINDOW, CHILDID_SELF);
		}
	}
...
void CMDIFrameWndEx::AdjustClientArea()
{
	...
	if (m_wndClientArea.m_hWnd != NULL)
	{
...
{
	if (m_wndClientArea.GetMDITabs().m_hWnd == NULL)
	{
		Default();
		return;
	}
	...
	if (pWnd->m_hWnd == m_wndClientArea.m_hWnd)
	{
		...
		{
			if (SendMessage(AFX_WM_TOOLBARMENU, (WPARAM) m_hWnd, MAKELPARAM(point.x, point.y)))
			{
				m_dockManager.OnPaneContextMenu(point);
			}
		}
	}
	else if (pWnd->m_hWnd == m_wndClientArea.GetMDITabs().m_hWnd)
	{
...
BOOL CMDIFrameWndEx::OnShowPopupMenu(CMFCPopupMenu* pMenuPopup)
{
	if (GetGlobalData()->IsAccessibilitySupport() && pMenuPopup != NULL)
	{
		::NotifyWinEvent(EVENT_SYSTEM_MENUPOPUPSTART, pMenuPopup->m_hWnd, OBJID_WINDOW , CHILDID_SELF);
	}
...
void CMDIFrameWndEx::RegisterAllMDIChildrenWithTaskbar(BOOL bRegister)
{
	...
	else
	{
		...
		{
			pTaskbarList3->SetThumbnailClip(m_hWnd, NULL);
		}
	}
}
