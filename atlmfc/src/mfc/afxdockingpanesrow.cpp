...
void CDockingPanesRow::MovePane(CPane* pControlBar, int nAbsoluteOffset, HDWP& /*hdwp*/)
{
	ASSERT_VALID(this);
	ASSERT_VALID(pControlBar);
	...
	if (IsHorizontal())
	{
		...
		rectBarWnd.left = rectRow.left + nAbsoluteOffset;
		rectBarWnd.right = rectBarWnd.left + nBarLength;
	}
	else
	{
		nBarLength = rectBarWnd.Height();
		rectBarWnd.top = rectRow.top + nAbsoluteOffset;
