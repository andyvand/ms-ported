...
#else
			USES_CONVERSION;
			VERIFY_STACK_FOR_A2W_RETURN(strAppID, FALSE);
#pragma warning(suppress: 6255)	// VERIFY_STACK_FOR_A2W_RETURN ensures enough stack space for below A2W conversion
			...
			{
#ifndef __MINGW32__
                TRACE("Error converting appID string, initialization canceled");
#endif
...
#else
		USES_CONVERSION;
		VERIFY_STACK_FOR_A2W_RETURN(strTitle, FALSE);
#pragma warning(suppress: 6255)	// VERIFY_STACK_FOR_A2W_RETURN ensures enough stack space for below A2W conversion
		LPCWSTR lpTitle = A2W(strTitle);
		if (lpTitle == NULL || FAILED(InitPropVariantFromString(lpTitle, &var)))
		{
			return FALSE;
		}
#endif
...
#else
	USES_CONVERSION;
	VERIFY_STACK_FOR_A2W_RETURN(lpcszDestinationPath, FALSE);
#pragma warning(suppress: 6255)	// VERIFY_STACK_FOR_A2W_RETURN ensures enough stack space for below A2W conversion
	LPWSTR lpDestPath = A2W(lpcszDestinationPath);
	if (lpDestPath == NULL)
	{
#ifndef __MINGW32__
        TRACE("Unable to convert destination path string");
#endif
...
BOOL CJumpList::CommitList()
{
	...
	for (POSITION pos = m_mapDestinations.GetStartPosition(); pos != NULL;)
	{
		...
		USES_CONVERSION;
		HRESULT hr;
		if (VERIFY_STACK_FOR_A2W(strCategory))
		{
#pragma warning(suppress: 6255 6263)	// VERIFY_STACK_FOR_A2W ensures enough stack space for below A2W conversion
			LPWSTR lpCategory = A2W(strCategory);
			hr = m_destListPtr->AppendCategory(lpCategory, pFinalCollection);
		}
		else
		{
			hr = E_OUTOFMEMORY;
...
#else
	USES_CONVERSION;
#pragma warning(suppress: 6255)	// There is no workaround for below conversion. We need to keep the behavior as-is.
