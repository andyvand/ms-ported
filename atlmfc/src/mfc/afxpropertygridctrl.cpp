...
void CMFCPropertyGridProperty::Expand(BOOL bExpand)
{
	...
	if (m_pWndList != NULL && m_pWndList->m_hWnd != NULL)
	{
		ASSERT_VALID(m_pWndList);
...
void CMFCPropertyGridProperty::Reposition(int& y)
{
	...
	if (bShowProperty)
	{
		...
		if (IsWindow(m_pWndList->m_ToolTip.m_hWnd))
		{
...
BOOL CMFCPropertyGridProperty::OnUpdateValue()
{
	...
	ASSERT_VALID(m_pWndList);
	ASSERT(::IsWindow(m_pWndInPlace->m_hWnd));
...
BOOL CMFCPropertyGridProperty::OnClickValue(UINT uiMsg, CPoint point)
{
	...
	CPoint ptEdit = point;
	::MapWindowPoints( m_pWndList->m_hWnd, m_pWndInPlace->m_hWnd, &ptEdit, 1);
...
BOOL CMFCPropertyGridProperty::PushChar(UINT nChar)
{
	...
	if (::GetCapture() == m_pWndList->m_hWnd)
	{
		ReleaseCapture();
	}
...
void CMFCPropertyGridProperty::Show(BOOL bShow/* = TRUE*/, BOOL bAdjustLayout/* = TRUE*/)
{
	...
	if (bAdjustLayout && m_pWndList != NULL && m_pWndList->m_hWnd != NULL)
	{
		ASSERT_VALID(m_pWndList);
		m_pWndList->AdjustLayout();
	}
...
void CMFCPropertyGridCtrl::Init()
{
	...
	hdItem.fmt = HDF_LEFT;
	hdItem.pszText = const_cast<LPTSTR>(_T("Property"));
	hdItem.cchTextMax = 100;

	GetHeaderCtrl().InsertItem(0, &hdItem);

	hdItem.pszText = const_cast<LPTSTR>(_T("Value"));
	hdItem.cchTextMax = 100;
	...
	if (m_pSel != NULL && m_pSel->m_bInPlaceEdit && m_pSel->m_bEnabled)
	{
		...
		if (pMsg->message == WM_KEYDOWN && (!bIsDroppedDown || pMsg->wParam == VK_RETURN))
		{
			...
			case VK_RETURN:
				...
				if (bIsDroppedDown)
				{
					HWND hwndInplace = m_pSel->m_pWndInPlace->m_hWnd;
			...
			case VK_UP:
				...
				else if (::IsWindow(m_pSel->m_pWndInPlace->m_hWnd))
				{
		...
		else if (pMsg->message >= WM_MOUSEFIRST && pMsg->message <= WM_MOUSELAST)
		{
			...
			if (m_pSel->m_pWndSpin != NULL)
			{
				...
				ASSERT(m_pSel->m_pWndSpin->m_hWnd != NULL);
...
void CMFCPropertyGridCtrl::SetScrollSizes()
{
	...
	if (m_wndScrollVert.m_hWnd == NULL)
	{
		return;
	}
...
{
	if (m_pSel != NULL && pScrollBar->m_hWnd != NULL && m_pSel->m_pWndSpin->m_hWnd == pScrollBar->m_hWnd)
	{
		return;
	}
...
{
	if (nBar == SB_HORZ || m_wndScrollVert.m_hWnd == NULL)
	{
		return NULL;
	}
...
HBRUSH CMFCPropertyGridCtrl::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (m_pSel != NULL && pWnd->m_hWnd == m_pSel->m_pWndInPlace->m_hWnd)
	{
...
void CMFCPropertyGridCtrl::UpdateColor(COLORREF color)
{
	...
	if (color == (COLORREF)-1 && pColorProp->m_pWndInPlace != NULL && ::IsWindow(pColorProp->m_pWndInPlace->m_hWnd))
	{
...
{
	ASSERT_VALID(this);
	ASSERT(m_hWnd == NULL); // Should be called before window create
...
{
	if (m_ToolTip.m_hWnd == NULL || pNMH->hwndFrom != m_ToolTip.m_hWnd)
	{
		return FALSE;
	}
...
{
	if (m_pSel == NULL || m_pSel->m_pWndInPlace->m_hWnd == NULL)
	{
		return FALSE;
	}
...
void CMFCPropertyGridCtrl::NotifyAccessibility(CMFCPropertyGridProperty* pProp)
{
	...
	::NotifyWinEvent (EVENT_OBJECT_FOCUS, m_hWnd, OBJID_CLIENT, (LONG)lParam);
}
...
HRESULT CMFCPropertyGridCtrl::get_accName(VARIANT varChild, BSTR *pszName)
{
	...
	if (m_pAccProp != NULL)
	{
		...
		{
			strName = m_pAccProp->FormatProperty();
		}
		else
		{
			strName = m_pAccProp->GetName();
		}
