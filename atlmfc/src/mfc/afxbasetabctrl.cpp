...
{
	if (pNewWnd->m_hWnd != NULL && pNewWnd->GetDlgCtrlID() == -1)
	{
		ASSERT(FALSE);
...
void CMFCBaseTabCtrl::InsertTab(CWnd* pNewWnd, LPCTSTR lpszTabLabel, int nInsertAt, UINT uiImageId, BOOL bDetachable)
{
	...
	if (!m_bHideInactiveWnd && pActiveWnd != NULL && pActiveWnd->m_hWnd != NULL)
	{
...
int CMFCBaseTabCtrl::GetTabFromHwnd(HWND hwnd) const
{
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		if (pWrapper != NULL)
		{
			...
			if (pWrappedWnd != NULL && pWrappedWnd->m_hWnd == hwnd)
			{
				return i;
			}
		}
...
void CMFCBaseTabCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_iHighlighted >= 0 && m_iPressed < 0 && !m_bReadyToDetach)
	{
		...
		if (pWnd != NULL && pWnd->m_hWnd != m_hWnd)
		{
			ReleaseCapture();
			m_iHighlighted = -1;
...
void CMFCBaseTabCtrl::FireChangeActiveTab(int nNewTab)
{
	...
	if (GetGlobalData()->IsAccessibilitySupport() && nNewTab >= 0 && nNewTab < (int)m_arTabs.GetSize())
	{
		...
		::NotifyWinEvent(EVENT_OBJECT_SELECTION, m_hWnd, OBJID_CLIENT , nNewTab + 1);
	}
}
...
BOOL CMFCBaseTabCtrl::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	if (pNMH->hwndFrom == m_pToolTipClose->m_hWnd)
	{
