...
BOOL CMFCDesktopAlertWnd::CommonCreate(CPoint ptPos, CMFCDesktopAlertWndInfo* pParams)
{
	...
	if (pWndForeground->m_hWnd != NULL)
	{
		pWndForeground->SetForegroundWindow();
	}

	if (pWndFocus->m_hWnd != NULL)
	{
		pWndFocus->SetFocus();
	}
...
BOOL CMFCDesktopAlertWnd::ProcessCommand(HWND hwnd)
{
	...
	if (hwnd == m_btnClose.m_hWnd)
	{
		SendMessage(WM_CLOSE);
		return TRUE;
	}

	if (hwnd == m_btnMenu.m_hWnd && m_hMenu != NULL)
	{
		CRect rectMenuBtn;
		...
		{
			nMenuResult = ::TrackPopupMenu(m_hMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, x, y, 0, m_hWnd, NULL);
		}
...
void CMFCDesktopAlertWnd::OnDraw(CDC* pDC)
{
	...
	else if (m_btnClose.m_hWnd != NULL)
	{
		pWndBtn = &m_btnClose;
	}
