...
{
	CWinAppEx* pApp = DYNAMIC_DOWNCAST(CWinAppEx, AfxGetApp());
	if (pApp != NULL && ::IsWindow(m_pFrame->m_hWnd))
	{
		WINDOWPLACEMENT wp;
...
void CFrameImpl::LoadTearOffMenus()
{
	...
	if (pApp != NULL)
	{
		strProfileName = pApp->GetRegSectionPath();
	}
...
BOOL CFrameImpl::ProcessKeyboard(int nKey, BOOL* pbProcessAccel)
{
	...
	if (pActivePopupMenu != NULL)
	{
		...
		{
			BOOL bIsFloatyActive = (pFocus->m_hWnd != NULL &&
				(pActivePopupMenu->IsChild(pFocus) || pFocus->m_hWnd == pActivePopupMenu->m_hWnd));
		...
		if (pFocus->m_hWnd != NULL && pActivePopupMenu->IsChild(pFocus))
		{
			return FALSE;
		}
...
BOOL CFrameImpl::ProcessMouseClick(UINT uiMsg, POINT pt, HWND hwnd)
{
	...
	if (uiMsg == WM_NCRBUTTONUP && hwnd == m_pFrame->m_hWnd && IsOwnerDrawCaption())
	{
		...
		if (nHit == HTCAPTION || nHit == HTSYSMENU || nHit == HTMINBUTTON || nHit == HTMAXBUTTON || nHit == HTCLOSE)
		{
			...
			if (pMenu->GetSafeHmenu() != NULL && ::IsMenu(pMenu->GetSafeHmenu()))
			{
				...
				UINT uiRes = ::TrackPopupMenu (pMenu->GetSafeHmenu(), TPM_LEFTBUTTON | TPM_RETURNCMD, pt.x, pt.y, 0, m_pFrame->m_hWnd, NULL);
				if (uiRes != 0)
				{
...
BOOL CFrameImpl::OnShowPopupMenu(CMFCPopupMenu* pMenuPopup, CFrameWnd* /*pWndFrame*/)
{
	...
	if (pMenuPopup != NULL && !CMFCToolBar::IsCustomizeMode())
	{
		...
		if (pTopLevelBar != NULL && !pTopLevelBar->IsKindOf(RUNTIME_CLASS(CMFCPopupMenuBar)))
		{
			...
			if (pTopLevelBar->IsDocked() && ::GetFocus() != pTopLevelBar->m_hWnd && CMFCPopupMenu::GetForceMenuFocus())
			{
...
BOOL CFrameImpl::OnNcPaint()
{
	...
	if (m_pRibbonBar->m_hWnd != NULL && ((m_pRibbonBar->IsWindowVisible() || IsFullScreeen()) || !m_pFrame->IsWindowVisible()) && m_pRibbonBar->IsReplaceFrameCaption())
	{
...
void CFrameImpl::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	...
	if ((m_pFrame->GetStyle() & WS_CAPTION) == 0 || (m_pFrame->GetStyle() & WS_BORDER) == 0 || bIsRibbonCaption)
	{
		...
		if (m_pRibbonBar->m_hWnd != NULL && m_pRibbonBar->IsReplaceFrameCaption() && !GetGlobalData()->IsDwmCompositionEnabled())
		{
...
BOOL CFrameImpl::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS FAR* lpncsp)
{
	...
	if (m_pRibbonStatusBar->m_hWnd != NULL && (m_pRibbonStatusBar->IsWindowVisible() || !m_pFrame->IsWindowVisible()))
	{
...
void __stdcall CFrameImpl::AddFrame(CFrameWnd* pFrame)
{
	...
	for (POSITION pos = m_lstFrames.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pListFrame->m_hWnd == pFrame->m_hWnd)
		{
			return;
		}
	}
...
UINT CFrameImpl::OnNcHitTest(CPoint point)
{
	...
	{
		LRESULT res = 0;
		_AfxDwmDefWindowProc(m_pFrame->m_hWnd, WM_NCHITTEST, 0, MAKELPARAM(point.x, point.y), &res);
		return (UINT)res;
	}
