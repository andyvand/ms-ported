...
void CDockingManager::AdjustDockingLayout(HDWP hdwp)
{
	...
	if (pWndTopLevelFrame->m_hWnd != NULL && ::IsWindow(pWndTopLevelFrame->m_hWnd) && pWndTopLevelFrame->IsIconic())
	{
		return;
	}
...
BOOL CDockingManager::ReplacePane(CDockablePane* pOriginalBar, CDockablePane* pNewBar)
{
	...
	if (pOriginalBar->IsTabbed())
	{
		...
		if (pTabWnd != NULL)
		{
			...
			int nTabNum = pTabWnd->GetTabFromHwnd(pOriginalBar->m_hWnd);
...
void CDockingManager::RedrawAllMiniFrames()
{
	for (POSITION pos = m_lstMiniFrames.GetHeadPosition (); pos != NULL;)
	{
		...
		if (pWnd->m_hWnd != NULL && pWnd->IsWindowVisible())
		{
			ASSERT_VALID (pWnd);
