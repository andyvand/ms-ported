...
void CMFCRibbonButton::OnCalcTextSize(CDC* pDC)
{
	...
	if (m_bAlwaysShowDescription && !m_strDescription.IsEmpty())
	{
		...
		// Description will be drawn below the text(in case of text on right only)
		int nTextHeight = 0;
		int nTextWidth = 0;
	...
	if (!IsApplicationButton())
	{
		if (!rectMenuArrow.IsRectEmpty())
		{
			...
			if (IsMenuMode())
			{
				...
				if (pTopLevelRibbon->m_hWnd != NULL)
				{
...
void CMFCRibbonButton::OnShowPopupMenu()
{
	...
	if (pTopLevelRibbon->GetParentFrame()->m_hWnd != AfxGetMainWnd()->m_hWnd)
	{
	...
	if (pMenuActive != NULL && pMenuActive->m_hWnd != pWndParent->GetParent()->m_hWnd)
	{
...
int CMFCRibbonButton::DrawRibbonText(CDC* pDC, const CString& strText, CRect rectText, UINT uiDTFlags, COLORREF clrText)
{
	...
	{
		const BOOL bIsZoomed = GetParentRibbonBar()->m_hWnd != NULL && GetParentRibbonBar()->GetParent()->IsZoomed();
