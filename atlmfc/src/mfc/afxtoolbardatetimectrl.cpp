...
SIZE CMFCToolBarDateTimeCtrl::OnCalculateSize(CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	...
	if (bHorz)
	{
		if (m_pWndDateTime->m_hWnd != NULL && !m_bIsHidden)
		{
	...
	{
		if (m_pWndDateTime->m_hWnd != NULL && (m_pWndDateTime->GetStyle() & WS_VISIBLE))
		{
...
{
	if (m_pWndDateTime->m_hWnd == NULL || (m_pWndDateTime->GetStyle() & WS_VISIBLE) == 0)
	{
		return;
	}
...
{
	if (m_pWndDateTime->m_hWnd == NULL || (m_pWndDateTime->GetStyle() & WS_VISIBLE) == 0 || m_rect.IsRectEmpty())
	{
		return;
	}
...
BOOL CMFCToolBarDateTimeCtrl::NotifyCommand(int iNotifyCode)
{
	...
	switch(iNotifyCode)
	{
	case (int)LOWORD(DTN_DATETIMECHANGE):
	case (int)DTN_DATETIMECHANGE:
		{
			m_dwTimeStatus = m_pWndDateTime->GetTime(m_time);
...
{
	return m_pWndDateTime->m_hWnd != NULL && (m_pWndDateTime->GetStyle() & WS_VISIBLE);
}
