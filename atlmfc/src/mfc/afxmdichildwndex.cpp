...
void CMDIChildWndEx::RegisterTaskbarTab(CMDIChildWndEx* pWndBefore)
{
	...
	if (pTaskbarList3)
	{
		...
		HRESULT hr = pTaskbarList3->RegisterTab(m_tabProxyWnd.m_hWnd, pTopLevel->m_hWnd);
		if (FAILED(hr))
		{
		...
		if(pWndBefore == NULL)
		{
			TRACE1("Setting of tab order failed\n", hr);
			UnregisterTaskbarTab();
			return;
		}

		CMDITabProxyWnd* pProxyWnd = pWndBefore->GetTabProxyWnd();

		hr = pTaskbarList3->SetTabOrder(m_tabProxyWnd.m_hWnd, pProxyWnd->m_hWnd);
		if (FAILED(hr))
		{
		...
		_AfxDwmSetWindowAttribute(m_tabProxyWnd.m_hWnd, DWMWA_HAS_ICONIC_BITMAP, &bHasIconicBitmap, sizeof(BOOL));
		_AfxDwmSetWindowAttribute(m_tabProxyWnd.m_hWnd, DWMWA_FORCE_ICONIC_REPRESENTATION, &bHasIconicBitmap, sizeof(BOOL));
...
{
	return m_tabProxyWnd.m_hWnd != NULL;
}
...
BOOL CMDIChildWndEx::InvalidateIconicBitmaps()
{
	...
	_AfxDwmInvalidateIconicBitmaps(m_tabProxyWnd.m_hWnd);
...
void CMDIChildWndEx::SetTaskbarTabOrder(CMDIChildWndEx* pWndBefore)
{
	...
	if (m_tabProxyWnd.m_hWnd != NULL)
	{
		...
		HWND hWndBefore = pWndBefore != NULL ? pWndBefore->GetTabProxyWnd()->m_hWnd : NULL;
		if (pTaskbarList != NULL)
		{
			pTaskbarList->SetTabOrder(m_tabProxyWnd.m_hWnd, hWndBefore);
		}
	}
}
...
void CMDIChildWndEx::SetTaskbarTabProperties(DWORD dwFlags)
{
	...
	if (m_tabProxyWnd.m_hWnd != NULL)
	{
		...
		{
			HRESULT hr = spTaskbarList4->SetTabProperties(m_tabProxyWnd.m_hWnd, (STPFLAG)dwFlags);
...
void CMDIChildWndEx::SetTaskbarTabActive()
{
	...
	if (pTaskbarList3)
	{
		...
		pTaskbarList3->SetTabActive(m_tabProxyWnd.m_hWnd, pParentFrame->m_hWnd, 0);
	}
}
void CMDIChildWndEx::OnMDIActivate(BOOL bActivate, CWnd* pActivateWnd, CWnd* pDeactivateWnd)
{
	...
	if (!bActivating)
	{
		...
		{
			ASSERT_VALID(m_pMDIFrame);
			m_pMDIFrame->m_wndClientArea.SetActiveTab(pActivateWnd->m_hWnd);
		}
...
void CMDIChildWndEx::ActivateFrame(int nCmdShow)
{
	...
	if (CMDIFrameWndEx::m_bDisableSetRedraw || pWndParent->m_hWnd == NULL)
	{
		if ((GetStyle() & WS_SYSMENU) == 0)
		{
...
HICON CMDIChildWndEx::GetFrameIcon() const
{
	...
	if (hIcon == NULL)
	{
		hIcon = (HICON)(LONG_PTR) GetClassLongPtr(m_hWnd, GCLP_HICONSM);
	}
			...
			else
			{
				...
				if (pView != NULL && GetParentFrame()->GetActiveFrame() == this && m_bActivating)
				{
					...
					if (pFrame->m_hWnd != NULL)
					{
						...
						if (pChildWnd->m_hWnd != NULL && pChildWnd->IsKindOf(RUNTIME_CLASS(CSplitterWnd)))
						{
...
void CMDIChildWndEx::UnregisterTaskbarTab(BOOL bCheckRegisteredMDIChildCount)
{
	...
	if (m_tabProxyWnd.m_hWnd != NULL)
	{
		...
		if (pTaskbarList3)
		{
			pTaskbarList3->UnregisterTab(m_tabProxyWnd.m_hWnd);
		}
		if (m_tabProxyWnd.m_hWnd != NULL)
		{
		...
		if (bCheckRegisteredMDIChildCount)
		{
			...
			if (pTopLevel != NULL && pTopLevel->GetRegisteredWithTaskBarMDIChildCount() == 0)
			{
				...
				{
					pTaskbarList3->SetThumbnailClip(pTopLevel->m_hWnd, NULL);
				}
...
void CMDIChildWndEx::OnDestroy()
{
	...
	if (m_pTabbedControlBar != NULL && CWnd::FromHandlePermanent(m_pTabbedControlBar->m_hWnd) != NULL)
	{
...
void CMDIChildWndEx::OnSendIconicThumbnail(WPARAM, LPARAM)
{
	...
	_AfxDwmSetIconicThumbnail(m_hWnd, hBitmap, DWM_SIT_DISPLAYFRAME);
}
...
void CMDIChildWndEx::OnSendIconicLivePreviewBitmap(WPARAM, LPARAM)
{
	...
	HBITMAP hBitmap = (HBITMAP)(dcThumbnail.GetCurrentBitmap()->m_hObject);
	_AfxDwmSetIconicLivePreviewBitmap(m_hWnd, hBitmap, &ptClient, DWM_SIT_DISPLAYFRAME);
}
...
CWnd* CMDIChildWndEx::GetTaskbarPreviewWnd()
{
	...
	if (pWnd->m_hWnd != NULL)
	{
		...
		if (pParent != this && pParent->m_hWnd != NULL && pParent->IsKindOf(RUNTIME_CLASS(CSplitterWnd)))
		{
			pWnd = pParent;
		}
	}
...
BOOL CMDIChildWndEx::SetTaskbarThumbnailClipRect(CRect rect)
{
	...
	HRESULT hr = pTaskbarList3->SetThumbnailClip(pTopLevel->m_hWnd, rect.IsRectNull() || rect.IsRectEmpty() ? NULL : &rect);
...
void CMDIChildWndEx::ActivateTopLevelFrame()
{
	...
	pTopLevel->PostMessage(AFX_WM_AFTER_TASKBAR_ACTIVATE, (WPARAM)bIsMinimized, (LPARAM)m_hWnd);
...
BOOL CMDITabProxyWnd::OnSendIconicThumbnail(int cx, int cy)
{
	...
	_AfxDwmSetIconicThumbnail(m_hWnd, hBitmap, 0);
...
BOOL CMDITabProxyWnd::OnSendIconicLivePreviewBitmap()
{
	...
	_AfxDwmSetIconicLivePreviewBitmap(m_hWnd, hBitmap, &ptClient, 0);
	DeleteObject(hBitmap);
	return FALSE;
}
