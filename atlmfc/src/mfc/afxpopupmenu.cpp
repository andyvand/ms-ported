...
void CMFCPopupMenu::RecalcLayout(BOOL /*bNotify*/)
{
	...
	else if (bIsTearOff)
	{
		...
		if (!CMFCToolBar::IsCustomizeMode() && m_wndToolTip.m_hWnd == NULL)
		{
	...
	if (bHightlightWasChanged && m_bScrollable && pMenuBar->m_iHighlighted >= 0)
	{
		...
		else
		{
			...
			if (iOffsetDelta != 0)
			{
				...
				if (m_bShowScrollBar && m_wndScrollBarVert.m_hWnd != NULL)
				{
...
void __stdcall CMFCPopupMenu::UpdateAllShadows(LPRECT lprectScreen)
{
	...
	{
		if (CWnd::FromHandlePermanent(pMenu->m_hWnd) != NULL)
		{
			ASSERT_VALID(pMenu);
...
void CMFCPopupMenu::SetScrollBar()
{
	...
	if (m_wndScrollBarVert.m_hWnd == NULL)
	{
...
{
	if (pScrollBar->m_hWnd != m_wndScrollBarVert.m_hWnd)
	{
...
HRESULT CMFCPopupMenu::get_accParent(IDispatch **ppdispParent)
{
	...
	*ppdispParent = NULL;
	...
	if (pWndParentBar->m_hWnd)
	{
		return AccessibleObjectFromWindow(pWndParentBar->m_hWnd, (DWORD)OBJID_CLIENT,
