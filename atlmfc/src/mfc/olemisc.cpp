...
{
	static constexpr const TCHAR* rgszFACILITY[] =
	{
		_T("FACILITY_NULL"),
		_T("FACILITY_RPC"),
	...
	static constexpr auto rgszFacilitySize = _countof(rgszFACILITY);

	ULONG scode = SCODE_FACILITY(sc);
	if(scode < rgszFacilitySize)
	{
		return rgszFACILITY[scode];
	}
...
AFX_STATIC HGLOBAL AFXAPI _AfxCopyGlobalMemory(HGLOBAL hDest, HGLOBAL hSource)
{
	...
	AFXASSUME(lpDest != NULL);
	AFXASSUME(lpSource != NULL);
	...
	LPSIZEL         lpSizelHim)
{
	...
	else if (lpszSrcOfCopy == NULL)
	{
		...
		if (sc == S_OK)
		{
			...
			if(SUCCEEDED(CreateBindCtx(0,&spbc)))
			{
...
RewriteInfo:
	...
	sc = ::WriteFmtUserTypeStg(lpStg, cfOld, lpszOld);
	...
	LPCTSTR lpszUserType, REFCLSID rclsid, REFCLSID rclsidNew)
{
	...
	if (sc != S_OK && lpszUserType != NULL)
	{
		...
		if(SUCCEEDED(::StringFromCLSID(rclsid, &lpOleStr)))
		{
