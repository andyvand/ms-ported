...
int CMFCToolBar::ReplaceButton(UINT uiCmd, const CMFCToolBarButton& button, BOOL bEvery/* = FALSE*/)
{
	ASSERT_VALID(this);
	...
	if ((!IsCustomizeMode() && !m_bAltCustomizeMode) || m_bLocked || m_bDisableCustomize)
	{
		...
		if (pButton->OnClick(this, FALSE /* No delay*/))
		{
			...
			if (!bIsRemoved && bWasDroppedDown != pButton->IsDroppedDown())
			{
				...
				if (nChildID > 0)
				{
					::NotifyWinEvent(EVENT_OBJECT_STATECHANGE, m_hWnd, OBJID_CLIENT, nChildID);
	...
	try
	{
		...
		if (lpbData != NULL)
		{
			...
			if (reg.CreateKey(strSection))
			{
				...
				{
					// Save original(before customization) state:
					SaveOriginalState(reg);
				}
				// Save original after reset state
...
HBRUSH CMFCToolBar::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pButton->IsOwnerOf(pWnd->m_hWnd))
		{
...
void CMFCToolBar::AdjustLocations()
{
	...
	if (m_Buttons.IsEmpty() || m_hWnd == NULL)
	{
		return;
	}
...
CMFCToolBarButton* CMFCToolBar::CreateDroppedButton(COleDataObject* pDataObject)
{
	...
	if (pButton->m_bDragFromCollection && pButton->GetImage() == -1 && pButton->m_strText.IsEmpty())
	{
		...
		// appearance, ask user about its properties:
		CMFCToolBarButtonCustomizeDialog dlg(pButton, m_pUserImages, this, 0, IsPureMenuButton(pButton));
...
void CMFCToolBar::OnSetFocus(CWnd* pOldWnd)
{
	...
	if (m_bLeaveFocus && pOldWnd != NULL && ::IsWindow(pOldWnd->m_hWnd) && DYNAMIC_DOWNCAST(CMFCToolBar, pOldWnd) == NULL &&
...
{
	ASSERT(m_hWnd == NULL); // Should not be created yet!
...
//
// Reset all toolbar images except user-defined to the default
//
{
	ASSERT_VALID(this);
...
HRESULT CMFCToolBar::get_accChild(VARIANT varChild, IDispatch **ppdispChild)
{
	...
	*ppdispChild = NULL;
	...
	if (varChild.vt == VT_I4 && varChild.lVal != CHILDID_SELF)
	{
		...
		if (pMenuButton != NULL && pMenuButton->GetPopupMenu()->m_hWnd != NULL)
		{
            return AccessibleObjectFromWindow(pMenuButton->GetPopupMenu()->m_hWnd, (DWORD)OBJID_CLIENT, IID_IAccessible, (void**)ppdispChild);
		}
	}
...
{
	if (m_hWnd == NULL || m_pCustomizeBtn == NULL)
	{
		return;
	}
