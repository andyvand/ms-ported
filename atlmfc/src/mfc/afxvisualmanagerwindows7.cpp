...
BOOL CMFCVisualManagerWindows7::IsWindowActive(CWnd* pWnd) const
{
	...
	if (hWnd != NULL)
	{
		if (!m_ActivateFlag.Lookup(pWnd->m_hWnd, bActive))
		{
			//ASSERT(FALSE);
			bActive = TRUE;
		}
...
BOOL CMFCVisualManagerWindows7::OnNcActivate(CWnd* pWnd, BOOL bActive)
{
	...
	if (pBar->m_hWnd == NULL || !pBar->IsWindowVisible() || !pBar->IsReplaceFrameCaption())
	{
		return FALSE;
	}
	...
	m_ActivateFlag[pWnd->m_hWnd] = bActive;
...
BOOL CMFCVisualManagerWindows7::OnNcPaint(CWnd* pWnd, const CObList& lstSysButtons, CRect rectRedraw)
{
	...
	if (pWnd->m_hWnd == NULL || m_hThemeWindow == NULL)
	{
		return FALSE;
	}
	...
	if (dc.GetSafeHdc() != NULL)
	{
		...
		if (bActive && IsOwnerDrawCaption() && pWnd->IsKindOf(RUNTIME_CLASS(CMDIChildWnd)))
		{
			...
			if (pParent)
			{
				...
				if (pActiveChild)
				{
					bActive = ((pActiveChild->m_hWnd == pWnd->m_hWnd) && IsWindowActive(pParent));
				}
			}
		}
