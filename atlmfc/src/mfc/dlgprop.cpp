...
BOOL CPropertyPage::OnSetActive()
{
	...
	if (m_bFirstSetActive)
	{
		...
		if (pPropSheet != NULL)
		{
			...
			if (pLayout != NULL && !pLayout->HasItem(m_hWnd) && !pLayout->IsEmpty() && pPropSheet->CanAddPageToDynamicLayout())
			{
				pLayout->AddItem(m_hWnd, CMFCDynamicLayout::MoveNone(), CMFCDynamicLayout::SizeHorizontalAndVertical(100, 100));
			}
		}
	}
...
INT_PTR CPropertySheet::DoModal()
{
	...
	// find parent HWND
	HWND hWndTop;
	HWND hWndParent = CWnd::GetSafeOwner_(m_pParentWnd->m_hWnd, &hWndTop);
...
BOOL CPropertySheet::Create(CWnd* pParentWnd, DWORD dwStyle, DWORD dwExStyle)
{
	...
	m_psh.hwndParent = pParentWnd->m_hWnd;
	...
	BOOL* pBool = hMem != NULL ? static_cast<BOOL*>(GlobalLock(hMem)) : NULL;
	if (pBool != NULL)
	{
		*pBool = TRUE;
		GlobalUnlock(hMem); 
...
LRESULT CPropertySheet::HandleInitDialog(WPARAM, LPARAM)
{
	...
	if (pDynamicLayout != NULL)
	{
		...
		for (CWnd *pChild = GetWindow(GW_CHILD); pChild->m_hWnd != NULL; pChild = pChild->GetWindow(GW_HWNDNEXT))
		{
			HWND hwndChild = pChild->m_hWnd;
