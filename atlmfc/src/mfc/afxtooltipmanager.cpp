...
BOOL __stdcall CTooltipManager::CreateToolTip(CToolTipCtrl*& pToolTip, CWnd* pWndParent, UINT nType)
{
	...
	if (pWndParent->m_hWnd != NULL && (pWndParent->GetExStyle() & WS_EX_LAYOUTRTL) != 0)
	{
	...
	if (pWndParent->m_hWnd != NULL && afxTooltipManager != NULL && afxTooltipManager->m_lstOwners.Find(pWndParent->m_hWnd) == NULL)
	{
		afxTooltipManager->m_lstOwners.AddTail(pWndParent->m_hWnd);
	}
...
void __stdcall CTooltipManager::DeleteToolTip(CToolTipCtrl*& pToolTip)
{
	if (pToolTip != NULL)
	{
		...
		if (pToolTip->m_hWnd != NULL)
		{
			HWND hwndParent = pToolTip->GetParent()->m_hWnd;
