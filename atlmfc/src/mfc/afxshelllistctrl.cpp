...
HRESULT CMFCShellListCtrl::DisplayParentFolder()
{
	...
	int nLevel = afxShellManager->GetParentItem(m_pidlCurFQ, info.pidlFQ); // lgtm [cpp/uninitializedptrfield] all items have been validated above
...
HRESULT CMFCShellListCtrl::EnumObjects(LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent)
{
	...
	if (SUCCEEDED(hRes) && pEnum != NULL)
	{
		...
		while (pEnum->Next(1, &pidlTemp, &dwFetched) == S_OK && dwFetched)
		{
			...
			//AddRef the parent folder so its pointer stays valid
...
void CMFCShellListCtrl::DoDefault(int iItem)
{
	...
	else
	{
		...
		IContextMenu *pcm;
		HRESULT hr = psfFolder->GetUIObjectOf(m_hWnd, 1, (LPCITEMIDLIST*)&pInfo->pidlRel, IID_IContextMenu, NULL, (LPVOID*)&pcm);
...
void CMFCShellListCtrl::OnSetColumns()
{
	...
	for (int iColumn = AFX_ShellList_ColumnName; iColumn <= AFX_ShellList_ColumnModified; iColumn++)
	{
		...
		ENSURE(strColumnName.LoadString(arColumnNameIDs[iColumn]));
...
void CMFCShellListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	IContextMenu* pcm;
	HRESULT hr = m_psfCurFolder->GetUIObjectOf(m_hWnd, nSelItems, (LPCITEMIDLIST*)pPidls, IID_IContextMenu, NULL, (LPVOID*)&pcm);
