...
void CFrameWnd::OnEnable(BOOL bEnable)
{
	...
	{
		DWORD dwProcessId = 0;
		::GetWindowThreadProcessId(pParentWnd->m_hWnd, &dwProcessId);
	...
	CCreateContext* pContext)
{
    ...
    } else if (!CreateEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle,
		rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
		pParentWnd->m_hWnd, hMenu, (LPVOID)pContext))
	{
		TRACE(traceAppMsg, 0, "Warning: failed to create CFrameWnd.\n");
		if (hMenu != NULL)
			DestroyMenu(hMenu);
...
void CFrameWnd::OnClose()
{
	...
	if (pApp != NULL && pApp->m_pMainWnd == this)
	{
		...
		// CloseAllDocuments has closed pDocument and is now orphaned.
		// If we don't change to NULL this will trigger an access violation below.
		pDocument = NULL;
...
void CFrameWnd::DestroyDockBars()
{
	...
	while (pos != NULL)
	{
		...
		if (pBar->IsDockBar())
		{
			listDockBars.AddTail( pBar->m_hWnd );
		}
		else
		{
			listCtrlBars.AddTail( pBar->m_hWnd );
		}
	}
...
void CFrameWnd::SetProgressBarPosition(int nProgressPos)
{
	...
	pTaskbarList->SetProgressValue(m_hWnd, nProgressPos - m_nProgressBarRangeMin,
...
void CFrameWnd::SetProgressBarState(TBPFLAG tbpFlags)
{
	...
	pTaskbarList->SetProgressState(m_hWnd, tbpFlags);
}
...
#ifdef UNICODE
	return SUCCEEDED(pTaskbarList->SetOverlayIcon(m_hWnd, hIcon, lpcszDescr));
#else
	USES_CONVERSION;
	VERIFY_STACK_FOR_A2W_RETURN(lpcszDescr, FALSE);
#pragma warning(suppress: 6255)	// VERIFY_STACK_FOR_A2W_RETURN ensures enough stack space for below A2W conversion
	LPWSTR lpwszDescr = A2W(lpcszDescr);
	return SUCCEEDED(pTaskbarList->SetOverlayIcon(m_hWnd, hIcon, lpwszDescr));
#endif
}
