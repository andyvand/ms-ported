	...
	TRY
	{
		...
		case AFX_EVENT::event:
			...
			if (bRange)
			{
				...
				const auto cArgs = ++dispparams.cArgs;
				
				dispparams.rgvarg = new VARIANT[cArgs];
				Checked::memcpy_s(dispparams.rgvarg, cArgs * sizeof(VARIANT), 
					pEvent->m_pDispParams->rgvarg, sizeof(VARIANT) * (cArgs-1));

				VARIANT* pvarID = &dispparams.rgvarg[cArgs-1];
				V_VT(pvarID) = VT_I4;
				V_I4(pvarID) = idCtrl;
			}
