...
void CDockablePane::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pMenu != NULL && CWnd::FromHandlePermanent(pMenu->m_hWnd) != NULL)
	{
...
void CDockablePane::ShowPane(BOOL bShow, BOOL bDelay, BOOL bActivate)
{
	...
	if (IsTabbed() && bShow && bActivate)
	{
		...
		pParentTab->SetActiveTab(pParentTab->GetTabFromHwnd(m_hWnd));
	}
}
...
int CDockablePane::OnSetText(LPCTSTR lpszText)
{
	...
	if (IsTabbed())
	{
		...
		{
			int iTab = pParentTabWnd->GetTabFromHwnd(m_hWnd);
...
void CDockablePane::OnSetFocus(CWnd* pOldWnd)
{
	...
	if (pParentMiniFrame != NULL)
	{
		pParentMiniFrame->SetLastFocusedPane(m_hWnd);
	}
}
