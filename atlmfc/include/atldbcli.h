		...
		else
		{
			switch (wType)
			{
				...
				case DBTYPE_HCHAPTER:
					IChapteredRowset *spChapteredRowset = (IChapteredRowset *)pRowset;
...
// Implementation
		...
		_In_ DBLENGTH nSize) throw()
	{
		...
		*/
		...
		if (SUCCEEDED(hr))
		{
			// translate the columns names to ordinals if necessary
			for( ULONG i = 0; i < nColumns; i++ )
				if( spColumnNames[i] != NULL ) // if a column name was given, translate it to ordinal
				{
					...
					{
						ATLASSERT(FALSE); // unable to match column name to an ordinal !!!
						return E_FAIL;
					}
				}
			...
			if (SUCCEEDED(hr))
			{
				for( ULONG i = 0; i < nColumns; i++ )
				{
					...
					if( spColumnNames[i] != NULL ) // if a column name was given, translate it to ordinal
					{
						...
						{
							ATLASSERT(FALSE); // unable to match parameter name to an ordinal !!!
							return E_FAIL;
						}
					}
				}
...
public:
		...
		_In_opt_ CBookmarkBase* pBookmark = NULL) throw()
	{
		...
		hr = CAccessorBase::BindEntries(&binding, 1, &hAccessor, nLength, (IAccessor *)spAccessor);
		if (FAILED(hr))
			return hr;
