...
{
public:
	void* Allocate(_In_ size_t nBytes)
    {
        alloc_size = nBytes;
        return malloc(nBytes);
    }

	void Free(_Inout_opt_ void* p)
    {
        alloc_size = 0;
        free(p);
    }

	void* Reallocate(_Inout_updates_bytes_opt_(nBytes) void* p, _In_ size_t nBytes)
    {
        alloc_size = nBytes;
        return realloc(p, nBytes);
    }

	size_t GetSize(_In_ void* p)
    {
        ((void)p);
...
{
public:
	_Ret_maybenull_ _Post_writable_byte_size_(nBytes) _ATL_DECLSPEC_ALLOCATOR void* Allocate(_In_ size_t nBytes) throw()
	{
		return( malloc( nBytes ) );
	}
