...
void AFXAPI DDX_FieldText(CDataExchange* pDX, int nIDC, CString& value,
	CRecordset* pRecordset);
void AFXAPI DDX_FieldText(_In_ CDataExchange* pDX, _In_ int nIDC, LPTSTR pstrValue,
...
public:
	...
	_Ret_range_(==, m_dwRowsetSize)
	DWORD GetRowsetSize() const;
	_Ret_range_(==, m_dwRowsFetched)
...
private:
	...
	inline void GetFieldValueEx(short nIndex, StringType &strValue, short nFieldType)
	{
		...
		SQLLEN nActualSize = GetData(m_pDatabase, m_hstmt, nIndex,
			nFieldType, pvData, nLen * sizeof(_CSTRING_CHAR_T),
		...
		else
		{
			...
			GetLongCharDataAndCleanup(m_pDatabase, m_hstmt, nIndex,
				nActualSize / (sizeof(_CSTRING_CHAR_T)), &pvData, nLen, strValue,
