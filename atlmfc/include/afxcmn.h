...
// Overridables
	...
	// Override to react to user cancelling drag.
...
// Attributes
	...
	// Gets the length of text in a particular pane without
	// retrieving the actual text.
...
// Attributes
	...
	_Success_(return == TRUE) BOOL GetColumnOrderArray(LPINT piArray, _In_ int iCount = -1) const;
...
// Operations
	...
	int HitTest(CPoint pt, _Out_opt_ UINT* pFlags = NULL) const;
...
// Attributes
	...
	int GetLine(_In_ int nIndex, _Inout_ LPTSTR lpszBuffer) const;
	// NOTE: Copied line does not contain null character!
	...
	BOOL SetTargetDevice(_In_opt_ HDC hDC, _In_ long lLineWidth);
