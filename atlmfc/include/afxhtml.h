...
// Implementation
	STDMETHOD(QueryInterface)(REFIID iid, __RPC__deref_out void **ppUnk) ATL_IUNKNOWN_NOEXCEPT
	{
		if (ppUnk == NULL)
		{
			return E_POINTER;
...
public:
	...
	HRESULT ExecCommand(const GUID *pGuid, long cmdID, long cmdExecOpt, VARIANT* pInVar=NULL, VARIANT* pOutVar=NULL) const
	{
		...
		if (spDoc)
		{
			::ATL::CComQIPtr<IOleCommandTarget> spCmdTarg = static_cast<IUnknown*>(spDoc);
			if (spCmdTarg)
			{
	...
	long QueryStatus(long cmdID) const
	{
		...
		if (spDoc)
		{
			...
			if (spCmdTarg)
			{
				OLECMD ocmd = { static_cast<ULONG>(cmdID), 0};
