...
public:
	...
	CStringData* Allocate(_In_ int nAllocLength, _In_ int nCharSize);

	// Free an existing CStringData
    void Free(_Inout_ CStringData* pData);

	// Change the size of an existing CStringData
	_Ret_maybenull_ _Post_writable_byte_size_(sizeof(CStringData) + nAllocLength*nCharSize)
    CStringData* Reallocate(_Inout_ CStringData* pData, _In_ int nAllocLength, _In_ int nCharSize);
...
struct CStringData
{
    CAfxStringMgr* pStringMgr;  // String manager for this CStringData
...
public:
	...
	void SetManager(_In_ CAfxStringMgr* pMgr) throw()
	{
...
public:
	explicit CSimpleStringT(_Inout_ CAfxStringMgr* pStringMgr)
	{
        CStringData* pData = NULL;

        pStringMgr = new CAfxStringMgr;
	...
	CSimpleStringT(
		...
		_Inout_ CAfxStringMgr* pStringMgr)
	{
        pStringMgr = new CAfxStringMgr;
	...
	CSimpleStringT& operator=(_In_ const CSimpleStringT& strSrc)
	{
		...
		if( pSrcData != pOldData)
		{
            if (pOldData == NULL)
            {
                CStringData* pNewData = CloneData( pSrcData );
                Attach( pNewData );
            } else if( pOldData->IsLocked() || pSrcData->pStringMgr != pOldData->pStringMgr )
			{
	...
	void Empty() throw()
	{
        ...
        CAfxStringMgr* pStringMgr = pOldData->pStringMgr;
    ...
    CAfxStringMgr* GetManager() const throw()
	{
        ...
        CAfxStringMgr* pStringMgr = GetData()->pStringMgr;
		return pStringMgr ? (CAfxStringMgr*)pStringMgr->Clone() : NULL;
	}
	...
	void SetManager(_Inout_ CAfxStringMgr* pStringMgr)
	{
		ATLASSERT( IsEmpty() );
...
protected:
	...
	ATL_NOINLINE static void __cdecl ThrowMemoryException()
	{
...
private:
	...
	static CStringData* __cdecl CloneData(_Inout_ CStringData* pData)
	{
        ...
        CAfxStringMgr* pNewStringMgr = (CAfxStringMgr *)pData->pStringMgr->Clone();
