...
{
public:
	CAtlStringMgr(_In_opt_ IAtlMemMgr* pMemMgr = NULL) :
		m_pMemMgr( pMemMgr )
	{
		m_nil.SetManager( (CAfxStringMgr*)this );
	}
	virtual ~CAtlStringMgr()
	{
		isInitialized = false;
	}
	void SetMemoryManager(_In_ IAtlMemMgr* pMemMgr)
	{
		ATLASSUME( m_pMemMgr == NULL );
		m_pMemMgr = pMemMgr;
	}

	static CAfxStringMgr* GetInstance()
	{
		static CWin32Heap strHeap( ::GetProcessHeap() );
		static CAtlStringMgr strMgr(&strHeap);

		return (CAfxStringMgr*)&strMgr;
	}
// IAtlStringMgr
public:
		...
		_In_ int nCharSize)
	{
		...
		pData->pStringMgr = (CAfxStringMgr*)this;
		pData->nRefs = 1;
		pData->nAllocLength = nAlignedChars;
		pData->nDataLength = 0;

		return( pData );
	}
	void Free(_In_ CStringData* pData)
	{
		...
		_In_ int nCharSize)
	{
		...
		pNewData->nAllocLength = nAlignedChars;
...
{
public:
	static int tclen(_In_z_ LPCSTR p)
	{
		ATLASSERT(p != NULL);
	...
	static _CharType* _strrev(_Inout_opt_z_ _CharType* psz)
	{
	...
	static _CharType* CharNext(_In_ const _CharType* p)
	{
		return AtlCharNext(p);
	}

	static int IsDigit(_In_ _CharType ch)
	{
		WORD type;
	...
	static int IsSpace(_In_ _CharType ch)
	{
		WORD type;
		GetStringTypeExA(GetThreadLocale(), CT_CTYPE1, &ch, 1, &type);
	...
	static int StringCompare(
		...
		_In_z_ const _CharType* pstrOther)
	{
	...
	static int StringSpanIncluding(
		...
		_In_z_ const _CharType* pstrSet)
	{
	...
	static _CharType* StringUppercase(_Inout_z_ _CharType* psz)
	{
		return CharUpperA( psz );
	}

	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringLowercase must be passed a buffer size")
	static _CharType* StringLowercase(_Inout_z_ _CharType* psz)
	{
	...
	static _CharType* StringReverse(_Inout_z_ _CharType* psz)
	{
		return _strrev( psz );
	}
	...
	static int GetBaseTypeLength(_In_z_ const wchar_t* pszSrc)
	{
		// Returns required buffer length in XCHARs
	...
	static void ConvertToOem(_Inout_z_ _CharType* pstrString)
	{
	...
	static void ConvertToAnsi(_Inout_z_ _CharType* pstrString)
	{
	...
	static void FloodCharacters(
		_In_ _CharType ch,
		_In_ int nLength,
		_Out_writes_bytes_all_(nLength) _CharType* pstr)
	{
	...
	static int SafeStringLen(_In_opt_z_ const char* psz)
	{
		// returns length in bytes
		return (psz != NULL) ? static_cast<int>(strlen(psz)) : 0;
	}

	static int SafeStringLen(_In_opt_z_ const wchar_t* psz)
	{
		// returns length in wchar_ts
		return (psz != NULL) ? static_cast<int>(wcslen(psz)) : 0;
	}

	static int GetCharLen(_In_opt_z_ const wchar_t*)
	{
		// returns char length
		return 1;
	}
	static int GetCharLen(_In_z_ const char* psz)
	{
...
public:
	static int tclen(_In_opt_z_ const wchar_t*)
	{
		return 1;
	}
	...
	_Ret_maybenull_z_ static wchar_t* _strrev(_Inout_opt_z_ wchar_t* psz)
	{
		// Optimize NULL, zero-length, and single-char case.
	...
	static wchar_t* CharNext(_In_ const wchar_t* p)
	{
		return AtlCharNext(p);
	}

	static int IsDigit(_In_ wchar_t ch)
	{
		WORD type;
	...
	static int IsSpace(_In_ wchar_t ch)
	{
		WORD type;
		GetStringTypeExW(0, CT_CTYPE1, &ch, 1, &type);
		return (type & C1_SPACE) == C1_SPACE;
	...
	static int StringCompare(
		...
		_In_z_ const wchar_t* pstrOther)
	{
	...
	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringUppercase must be passed a buffer size")
	static wchar_t* StringUppercase(_Inout_z_ wchar_t* psz)
	{
		CharUpperW(psz);
		return psz;
	}
	...
	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringUppercase must be passed a buffer size")
	static wchar_t* StringLowercase(_Inout_z_ wchar_t* psz)
	{
		CharLowerW(psz);
		return psz;
	}
	...
	_Ret_maybenull_z_ static wchar_t* StringReverse(_Inout_z_ wchar_t* psz)
	{
		return _strrev(psz);
	}
	...
	*/
	...
	static void FloodCharacters(
		_In_ wchar_t ch,
		_In_ int nLength,
		_Out_writes_all_(nLength) wchar_t* pstr)
	{
		// nLength is in XCHARs
...
public:
	...
	static CAfxStringMgr* GetDefaultManager()
	{
