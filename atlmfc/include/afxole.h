...
public:
	void Serialize(CArchive& ar) override;   // for Native data
	virtual ~CDocItem();
#ifdef _DEBUG
	void AssertValid() const override;
	void Dump(CDumpContext& dc) const override;
...
public:
	...
	void DeleteContents() override; // delete client items in list
	void Serialize(CArchive& ar) override;   // serialize items to file
	void PreCloseFrame(CFrameWnd* pFrame) override;
...
protected:
	...
	HRESULT OnLoadDocumentFromStream(IStream* pStream, DWORD grfMode) override;
...
#define _TEMP_OLE_MAXITEMNAME (ARRAYSIZE("Embedding ")+ARRAYSIZE("4294967295")-ARRAYSIZE(""))
	...
	void GetItemName(_Out_writes_z_(_TEMP_OLE_MAXITEMNAME) LPTSTR lpszItemName) const; // gets readable item name

#undef _TEMP_OLE_MAXITEMNAME
...
public:
	...
	void Release(OLECLOSE dwCloseOption = OLECLOSE_NOSAVE) override;
	void OnInsertMenus(CMenu* pMenuShared,
		LPOLEMENUGROUPWIDTHS lpMenuWidths) override;
	void OnRemoveMenus(CMenu *pMenuShared) override;
...
protected:
	...
	protected:
		...
		BOOL OnSetData(LPFORMATETC lpFormatEtc, LPSTGMEDIUM lpStgMedium,
			BOOL bRelease) override;
	};
...
public:
	...
	BOOL OnSaveDocument(LPCTSTR lpszPathName) override;
	void OnCloseDocument() override;
	LPOLEITEMCONTAINER GetContainer() override;
	LPMONIKER GetMoniker(OLEGETMONIKER nAssign) override;

	// special handling of error messages during save/load
	void ReportSaveLoadException(LPCTSTR lpszPathName,
		CException* e, BOOL bSaving, UINT nIDPDefault) override;
...
protected:
	...
	void OnShowViews(BOOL bVisible) override;

	void SaveToStorage(CObject* pObject = NULL) override;
...
// Advanced overridables
	LPUNKNOWN GetInterfaceHook(const void* piid) override;
...
public:
	...
	BOOL OnSaveDocument(LPCTSTR lpszPathName) override;
	void OnCloseDocument() override;
	void DeleteContents() override; // delete auto-delete server items
	LPMONIKER GetMoniker(OLEGETMONIKER nAssign) override;
	COleServerItem* OnGetLinkedItem(LPCTSTR lpszItemName) override;
...
protected:
	...
	HMENU GetDefaultMenu() override;     // return menu based on doc type
	HACCEL GetDefaultAccelerator() override; // return accel table based on doc type
...
public:
   using CMonikerFile::Open;
	...
	BOOL Open(IMoniker* pMoniker, CFileException* pError=NULL) override;
	...
	BOOL Open(LPCTSTR lpszURL, CFileException* pError=NULL) override;
	...
	// Returns the binding provided when the asynchronous transfer begins.
...
// Overridables
	...
	// IBinding and should nearly always be called when overridden.
...
protected:
	...
	BOOL PostBindToStream(CFileException* pError) override;
};
