...
public:
	...
	BOOL PreCreateWindow(CREATESTRUCT& cs) override;
	BOOL AllocElements(int nElements, int cbElement);
	void CalcInsideRect(CRect& rect, BOOL bHorz) const override;
...
protected:
	...
	BOOL OnChildNotify(UINT message, WPARAM, LPARAM, LRESULT*) override;
...
public:
	HRESULT get_accChild(VARIANT varChild, IDispatch **ppdispChild) override
	{
		return GetAccessibleChild(varChild, ppdispChild);
	}

	HRESULT get_accChildCount(long *pcountChildren) override
	{
	...
	HRESULT accHitTest(long xLeft, long yTop, VARIANT *pvarChild) override
	{
...
protected:
	...
	void OnDraw(CDC* pDC) override;      // default does nothing
	// special case override of child window creation
	BOOL Create(LPCTSTR, LPCTSTR, DWORD,
		const RECT&, CWnd*, UINT, CCreateContext*) override;
	BOOL PreTranslateMessage(MSG* pMsg) override;
	void OnActivateView(BOOL, CView*, CView*) override;
	void OnActivateFrame(UINT, CFrameWnd*) override;
...
protected:
	...
	void OnEndPrinting(CDC* pDC, CPrintInfo* pInfo = NULL) override;
...
// Implementation
public:
	void SetAttribDC(HDC hDC) override;  // Set the Attribute DC

protected:
	void SetOutputDC(HDC hDC) override;  // Set the Output DC -- Not allowed
	void ReleaseOutputDC() override;     // Release the Output DC -- Not allowed

public:
	virtual ~CMetaFileDC();

// Clipping Functions (use the Attribute DC's clip region)
	int GetClipBox(LPRECT lpRect) const override;
	BOOL PtVisible(int x, int y) const override;
			BOOL PtVisible(POINT point) const;
	BOOL RectVisible(LPCRECT lpRect) const override;

// Text Functions
    BOOL TextOut(int x, int y, LPCTSTR lpszString, int nCount) override;
    BOOL TextOut(int x, int y, const CString& str);
	BOOL ExtTextOut(int x, int y, UINT nOptions, LPCRECT lpRect,
				LPCTSTR lpszString, UINT nCount, _In_reads_opt_(nCount) LPINT lpDxWidths) override;
	...
	CSize TabbedTextOut(int x, int y, LPCTSTR lpszString, int nCount,
				int nTabPositions, LPINT lpnTabStopPositions, int nTabOrigin) override;
...
#undef DrawText
#undef DrawTextEx
	int _AFX_FUNCNAME(DrawText)(LPCTSTR lpszString, int nCount, LPRECT lpRect,
				UINT nFormat) override;
			int _AFX_FUNCNAME(DrawText)(const CString& str, LPRECT lpRect, UINT nFormat);

	int _AFX_FUNCNAME(DrawTextEx)(_In_reads_(nCount) LPTSTR lpszString, int nCount, LPRECT lpRect,
				UINT nFormat, LPDRAWTEXTPARAMS lpDTParams) override;
...
// Printer Escape Functions
	int Escape(int nEscape, int nCount, LPCSTR lpszInData, LPVOID lpOutData) override;

// Viewport Functions
	CPoint SetViewportOrg(int x, int y) override;
			CPoint SetViewportOrg(POINT point);
	CPoint OffsetViewportOrg(int nWidth, int nHeight) override;
	CSize SetViewportExt(int x, int y) override;
			CSize SetViewportExt(SIZE size);
	CSize ScaleViewportExt(int xNum, int xDenom, int yNum, int yDenom) override;
