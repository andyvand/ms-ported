...
public:
	...
	_Success_(1) _Acquires_lock_(this->m_sec) HRESULT Lock()
	{
		EnterCriticalSection(&m_sec);
		return S_OK;
	}
	_Success_(1) _Releases_lock_(this->m_sec) HRESULT Unlock()
	{
...
public :
	...
	HINSTANCE SetResourceInstance(_In_ HINSTANCE hInst)
	{
		return static_cast< HINSTANCE >(InterlockedExchangePointer((void**)&m_hInstResource, hInst));
	}

	bool AddResourceInstance(_In_ HINSTANCE hInst);
	bool RemoveResourceInstance(_In_ HINSTANCE hInst);
	HINSTANCE GetHInstanceAt(_In_ int i);
};
...
inline const ATLSTRINGRESOURCEIMAGE* AtlGetStringResourceImage(_In_ UINT id)
{
...
inline _CharType* AtlCharNext(_In_ const _CharType* p)
{
...
template <>
inline wchar_t* AtlCharNext<wchar_t>(_In_ const wchar_t* p)
{
...
template<typename CharType>
inline int AtlprintfT(_In_z_ _Printf_format_string_ const CharType* pszFormat,...)
{
	int retval=0;
	va_list argList;
...
#pragma warning(disable : 4793)
template<>
inline int AtlprintfT(_In_z_ _Printf_format_string_ const wchar_t* pszFormat,... )
{
	int retval=0;
	va_list argList;
	va_start( argList, pszFormat );
