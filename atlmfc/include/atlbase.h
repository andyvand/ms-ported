...
struct _ATL_OBJMAP_ENTRY30
{
		...
		_In_ DWORD dwFlags)
	{
		...
		if (pfnGetClassObject == NULL)
			return S_OK;
		HRESULT hRes = pfnGetClassObject((void *)pfnCreateInstance, __uuidof(IUnknown), (LPVOID*) &p);
		if (SUCCEEDED(hRes))
...
extern "C"
{
__declspec(selectany) _ATL_OBJMAP_ENTRY_EX* __pobjMapEntryFirst = NULL;
__declspec(selectany) _ATL_OBJMAP_ENTRY_EX* __pobjMapEntryLast = NULL;
}
...
// Private to avoid accidental use
	CComCritSecLock(_In_ const CComCritSecLock&);
	CComCritSecLock& operator=(_In_ const CComCritSecLock&);
};
...
{
public:
	static ULONG WINAPI Increment(_Inout_ LPLONG p)
	{
		return ::InterlockedIncrement(p);
	}
	static ULONG WINAPI Decrement(_Inout_ LPLONG p)
	{
...
// AtlAdvise (used by CComPtrBase) and AtlUnadvise are declared in <atlcomcli.h>, then defined below.
...
public:
	...
	CAutoVectorPtr(_Inout_ CAutoVectorPtr< T >& p)
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoVectorPtr(_Pre_maybenull_ T* p) :
		m_p( p )
	{
	}
	~CAutoVectorPtr()
	...
	CAutoVectorPtr< T >& operator=(_Inout_ CAutoVectorPtr< T >& p)
	{
		if(*this==p)
		{
			if(m_p == NULL)
	...
	// Allocate the vector
	bool Allocate(_In_ size_t nElements)
	{
		ATLASSUME( m_p == NULL );
...
public:
	...
	CAutoPtr(_Inout_ CAutoPtr< TSrc >& p)
	{
		m_p = p.Detach();  // Transfer ownership
	}
	CAutoPtr(_Inout_ CAutoPtr< T >& p)
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoPtr(_In_opt_ T* p) :
		m_p( p )
	{
	}
	~CAutoPtr()
	...
	template< typename TSrc >
	CAutoPtr< T >& operator=(_Inout_ CAutoPtr< TSrc >& p)
	{
		if(m_p==p.m_p)
		{
	...
	CAutoPtr< T >& operator=(_Inout_ CAutoPtr< T >& p)
	{
		if(*this==p)
		{
			if(this!=&p)
...
public:
	...
	template< typename TSrc >
	CAutoStackPtr(_Inout_ CAutoStackPtr< TSrc >& p)
	{
		m_p = p.Detach();  // Transfer ownership
	}
	CAutoStackPtr(_Inout_ CAutoStackPtr< T >& p)
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoStackPtr(_In_opt_ T* p) :
		m_p( p )
	{
	}
	~CAutoStackPtr()
	...
	// Templated version to allow pBase = pDerived
	template< typename TSrc >
	CAutoStackPtr< T >& operator=(_Inout_ CAutoStackPtr< TSrc >& p)
	{
		if(m_p==p.m_p)
		{
			// This means that two CAutoPtrs of two different types had the same m_p in them
	...
	CAutoStackPtr< T >& operator=(_Inout_ CAutoStackPtr< T >& p)
	{
		if(*this==p)
		{
			if(this!=&p)
...
Dest* static_cast_auto(_In_ const CAutoPtr< Src >& pSrc)
{
...
public:
	...
	static void* Allocate(_In_ size_t nBytes)
	{
#ifdef _WIN64
		if( nBytes > INT_MAX )
		{
	...
	static void Free(_In_opt_ void* p)
	{
		::CoTaskMemFree(p);
	}
};
...
public:
	...
	explicit CComHeapPtr(_In_ T* pData) :
...
public:
	CHandle();
	CHandle(_Inout_ CHandle& h);
	explicit CHandle(_In_opt_ HANDLE h);
	~CHandle();

	CHandle& operator=(_Inout_ CHandle& h);

	operator HANDLE() const;

	// Attach to an existing handle (takes ownership).
	void Attach(_In_ HANDLE h);
...
inline CHandle::CHandle(_Inout_ CHandle& h) :
	m_h( NULL )
{
	Attach( h.Detach() );
}

inline CHandle::CHandle(_In_opt_ HANDLE h) :
	m_h( h )
{
}
...
inline CHandle& CHandle::operator=(_Inout_ CHandle& h)
{
	if( this != &h )
	{
		if( m_h != NULL )
...
inline void CHandle::Attach(_In_ HANDLE h)
{
	ATLASSUME( m_h == NULL );
...
// Private to avoid accidental use
	CCritSecLock(_In_ const CCritSecLock&);
	CCritSecLock& operator=(_In_ const CCritSecLock&);
};
...
public:
	...
	CRegKey(_In_opt_ CAtlTransactionManager* pTM = NULL);
	CRegKey(_Inout_ CRegKey& key);
	explicit CRegKey(_In_ HKEY hKey);
	~CRegKey();

	CRegKey& operator=(_Inout_ CRegKey& key);
...
public:
	...
	LSTATUS EnumKey(
		...
		_Out_opt_ FILETIME* pftLastWriteTime = NULL);
	LSTATUS NotifyChangeKeyValue(
		_In_ BOOL bWatchSubtree,
		_In_ DWORD dwNotifyFilter,
		_In_ HANDLE hEvent,
		_In_ BOOL bAsync = TRUE);

	LSTATUS DeleteSubKey(_In_z_ LPCTSTR lpszSubKey);
	LSTATUS RecurseDeleteKey(_In_z_ LPCTSTR lpszKey);
	LSTATUS DeleteValue(_In_z_ LPCTSTR lpszValue);
};
#endif // _ATL_USE_WINAPI_FAMILY_DESKTOP_APP
...
public:
	...
	void DeleteNonAddRefThunk(_In_ IUnknown* pUnk)
	{
		CComCritSecLock<CComCriticalSection> lock(m_cs, false);
		HRESULT hr = lock.Lock();
		if (FAILED(hr))
	...
	void DeleteThunk(_In_ _QIThunk* p)
	{
		CComCritSecLock<CComCriticalSection> lock(m_cs, false);
		HRESULT hr = lock.Lock();
		if (FAILED(hr))
...
public :
	...
	virtual HRESULT GetGITPtr(_Outptr_ IGlobalInterfaceTable** ppGIT)
	{
...
#ifdef _ATL_USE_WINAPI_FAMILY_DESKTOP_APP
	virtual HRESULT AddCommonRGSReplacements(_Inout_ IRegistrarBase* /*pRegistrar*/) = 0;
...
#define DECLARE_REGISTRY_APPID_RESOURCEID(resid, appid) \
	...
	static HRESULT WINAPI UpdateRegistryAppId(_In_ BOOL bRegister) \
	{ \
...
inline HRESULT AtlGetGITPtr(_Outptr_ IGlobalInterfaceTable** ppGIT)
{
	if (ppGIT == NULL)
		return E_POINTER;
...
public :
	...
	HRESULT AddCommonRGSReplacements(_Inout_ IRegistrarBase* pRegistrar) override
	{
...
public :
	...
	static DWORD WINAPI MonitorProc(_In_ void* pv)
	{
...
public :
	...
	HRESULT Start(_In_ int nShowCmd)
	{
		T* pT = static_cast<T*>(this);
	...
	inline HRESULT RegisterAppId(_In_ bool bService = false)
	{
		if (!Uninstall())
			return E_FAIL;
	...
	void OnUnknownRequest(_In_ DWORD /*dwOpcode*/)
	{
		LogEvent(_T("Bad service request"));
	}

	void Handler(_In_ DWORD dwOpcode)
	{
		T* pT = static_cast<T*>(this);
	...
	void LogEventEx(
		...
		_In_ WORD type = EVENTLOG_INFORMATION_TYPE)
	{
	...
	void __cdecl LogEvent(
		_In_z_ _Printf_format_string_ LPCTSTR pszFormat, ...)
	{
	...
	void SetServiceStatus(_In_ DWORD dwState)
	{
...
static CComModule *_pModule = NULL;
class CComModule :
...
public :
	...
	CComModule()
	{
        ...
        void *mod = (void *)this;
		memcpy((void *)&_pModule, (const void *)&mod, sizeof(_pModule));
    ...
    HINSTANCE m_hInst;
	HINSTANCE& get_m_hInst() const
	{
		return _AtlBaseModule.m_hInst;
	}

	HINSTANCE m_hInstResource;
	HINSTANCE& get_m_hInstResource() const
	{
		return _AtlBaseModule.m_hInstResource;
	}

	void put_m_hInstResource(_In_ HINSTANCE h)
	{
		_AtlBaseModule.SetResourceInstance(h);
	}

	HINSTANCE SetResourceInstance(_In_ HINSTANCE h)
	{
	...
	HINSTANCE m_hInstTypeLib;
	HINSTANCE& get_m_hInstTypeLib() const
	{
		return _AtlComModule.m_hInstTypeLib;
	}
	void put_m_hInstTypeLib(_In_ HINSTANCE h)
	{
		_AtlComModule.m_hInstTypeLib = h;
	}

	HINSTANCE GetTypeLibInstance() const
	{
		return _AtlComModule.m_hInstTypeLib;
	}
...
#ifndef _ATL_NO_WIN_SUPPORT
	CRITICAL_SECTION m_csWindowCreate = _AtlWinModule.m_csWindowCreate.m_sec;
	CRITICAL_SECTION& get_m_csWindowCreate();

    _AtlCreateWndData* m_pCreateWndList = _AtlWinModule.m_pCreateWndList;
	_AtlCreateWndData*& get_m_pCreateWndList();
	void put_m_pCreateWndList(_In_ _AtlCreateWndData* p);
#endif // _ATL_NO_WIN_SUPPORT

	CRITICAL_SECTION m_csObjMap = _AtlComModule.m_csObjMap.m_sec;
	CRITICAL_SECTION& get_m_csObjMap();

	CRITICAL_SECTION m_csTypeInfoHolder = m_csStaticDataInitAndTypeInfo.m_sec;
	CRITICAL_SECTION m_csStaticDataInit = m_csStaticDataInitAndTypeInfo.m_sec;
	CRITICAL_SECTION& get_m_csStaticDataInit();
	void EnterStaticDataCriticalSection()
	{
		EnterCriticalSection(&m_csStaticDataInit);
	}

	void LeaveStaticDataCriticalSection()
	{
		LeaveCriticalSection(&m_csStaticDataInit);
	}

	DWORD dwAtlBuildVer = _AtlBaseModule.dwAtlBuildVer;
	DWORD& get_dwAtlBuildVer()
	{
		return _AtlBaseModule.dwAtlBuildVer;
	}

	const GUID* pguidVer = _AtlBaseModule.pguidVer;
	const GUID*& get_pguidVer()
	{
	...
	UINT m_nIndexQI = _AtlDebugInterfacesModule.m_nIndexQI;
	UINT& get_m_nIndexQI();
	void put_m_nIndexQI(_In_ UINT nIndex);

	UINT m_nIndexBreakAt = _AtlDebugInterfacesModule.m_nIndexBreakAt;
	UINT& get_m_nIndexBreakAt();
	void put_m_nIndexBreakAt(_In_ UINT nIndex);

	CSimpleArray<_QIThunk*>* m_paThunks = &_AtlDebugInterfacesModule.m_aThunks;
	CSimpleArray<_QIThunk*>* get_m_paThunks();
	HRESULT AddThunk(
		_Inout_ _Deref_pre_valid_ _Deref_post_valid_ IUnknown** pp,
		_In_z_ LPCTSTR lpsz,
		_In_ REFIID iid);
...
#ifdef _ATL_USE_WINAPI_FAMILY_DESKTOP_APP
	// Register/Revoke All Class Factories with the OS (EXE only)
	HRESULT RegisterClassObjects(_In_ DWORD dwClsContext, _In_ DWORD dwFlags);
	HRESULT RevokeClassObjects();

	// Registry support (helpers)
	HRESULT RegisterTypeLib();
	HRESULT RegisterTypeLib(_In_z_ LPCTSTR lpszIndex);
	HRESULT UnRegisterTypeLib();
	HRESULT UnRegisterTypeLib(_In_z_ LPCTSTR lpszIndex);
	...
	HRESULT UnregisterServer(
		_In_ BOOL bUnRegTypeLib,
		_In_opt_ const CLSID* pCLSID = NULL);
	HRESULT UnregisterServer(_In_opt_ const CLSID* pCLSID = NULL);
	...
	HRESULT DllRegisterServer(_In_ BOOL bRegTypeLib = TRUE) 
	{
		// registers object, typelib and all interfaces in typelib
		return RegisterServer(bRegTypeLib);
	}

	HRESULT DllUnregisterServer(_In_ BOOL bUnRegTypeLib = TRUE) 
	{
...
public:
	...
	explicit CComGITPtr(_In_ DWORD dwCookie)
	{
	...
	CComGITPtr(_Inout_ CComGITPtr&& rv)
	{
		m_dwCookie = rv.m_dwCookie;
		rv.m_dwCookie = 0;
	}

	CComGITPtr& operator=(_Inout_ CComGITPtr&& rv)
	{
		if (this != &rv)
		{
	...
	HRESULT Attach(_In_ DWORD dwCookie)
	{
	...
	HRESULT CopyTo(_Outptr_ T** pp) const
	{
		CComPtr<IGlobalInterfaceTable> spGIT;
		HRESULT hr = E_FAIL;
		hr = AtlGetGITPtr(&spGIT);
...
inline CRegKey::CRegKey(_In_opt_ CAtlTransactionManager* pTM) :
	m_hKey( NULL ), m_samWOW64(0), m_pTM(pTM)
{
}

inline CRegKey::CRegKey(_Inout_ CRegKey& key) :
	m_hKey( NULL )
{
...
inline CRegKey::CRegKey(_In_ HKEY hKey) :
...
inline CRegKey& CRegKey::operator=(_Inout_ CRegKey& key)
{
...
inline void CRegKey::Attach(_In_ HKEY hKey)
{
...
inline LSTATUS CRegKey::DeleteSubKey(_In_z_ LPCTSTR lpszSubKey)
{
	ATLASSUME(m_hKey != NULL);
...
inline LSTATUS CRegKey::DeleteValue(_In_z_ LPCTSTR lpszValue)
{
	ATLASSUME(m_hKey != NULL);
	return RegDeleteValue(m_hKey, (LPTSTR)lpszValue);
}

inline LSTATUS CRegKey::Close()
{
...
inline LSTATUS CRegKey::EnumKey(
	...
	_Out_writes_to_(*pnNameLength, *pnNameLength) _Post_z_ LPTSTR pszName,
	_Inout_ LPDWORD pnNameLength,
	_Out_opt_ FILETIME* pftLastWriteTime)
{
...
inline LSTATUS CRegKey::RecurseDeleteKey(_In_z_ LPCTSTR lpszKey)
{
	CRegKey key;
...
static inline UINT WINAPI AtlGetDirLen(_In_z_ LPCOLESTR lpszPathName)
{
	...
	_In_opt_z_ LPCOLESTR lpszIndex)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr) && (bstrHelpFile != (CComBSTR)0))
		{
	...
	_In_ HINSTANCE hInst)
{
	...
	for (int i = 0; i < pWinModule->m_rgWindowClassAtoms.GetSize(); i++)
		UnregisterClass((LPCTSTR)((unsigned long)pWinModule->m_rgWindowClassAtoms[i]), hInst);
...
inline void CComModule::put_m_pCreateWndList(_In_ _AtlCreateWndData* p)
{
...
inline void CComModule::put_m_nIndexQI(_In_ UINT nIndex)
{
...
inline void CComModule::put_m_nIndexBreakAt(_In_ UINT nIndex)
{
...
inline void CComModule::DeleteNonAddRefThunk(_In_ IUnknown* pUnk)
{
	_AtlDebugInterfacesModule.DeleteNonAddRefThunk(pUnk);
}

inline void CComModule::DeleteThunk(_In_ _QIThunk* p)
{
		...
		while (pEntry->pclsid != NULL)
		{
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				if (pEntry->pCF == NULL)
				{
					...
					if (pEntry->pCF == NULL)
					{
						hr = pEntry->pfnGetClassObject((void *)pEntry->pfnCreateInstance, __uuidof(IUnknown), (LPVOID*)&pEntry->pCF);
					}
				}
...
inline HRESULT CComModule::RegisterTypeLib(_In_z_ LPCTSTR lpszIndex)
{
...
inline HRESULT CComModule::UnRegisterTypeLib(_In_z_ LPCTSTR lpszIndex)
{
...
inline HRESULT CComModule::UnregisterServer(_In_opt_ const CLSID* pCLSID /*= NULL*/)
{
			...
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				...
				if (pCache->pCF == NULL)
				{
					...
					if (pCache->pCF == NULL)
					{
						...
						hr = pEntry->pfnGetClassObject((void *)pEntry->pfnCreateInstance, __uuidof(IUnknown), reinterpret_cast<void**>(&factory));
						if (SUCCEEDED(hr))
						{
