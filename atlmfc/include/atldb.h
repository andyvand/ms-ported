...
public:
	...
	static void FreeDBIDs(_In_ DBID* pdbidSrc)
	{
		switch( pdbidSrc->eKind )
		{
...
public:
	...
	bool operator==(_In_ const CPropColID& colId) const
	{
...
EXIT:
		...
		_In_opt_ CUtlPropsBase* pCopyMe = NULL)
	{
		...
		// With in the UPROPS Structure allocate and initialize the
		...
		_Inout_ DBPROP* pDBProp)
	{
		...
		if constexpr (_Has_CreateAccessor<T>::value)
		{
			...
			case DBPROP_UPDATABILITY:
				...
				// Hold off on this for now, it appears to be causing a GPF because
...
public:
	...
	~CSimpleRow()
	{
		m_pData = NULL;	// Data will be freed separately
	}
	DWORD AddRefRow()
	{
	...
	for (DBORDINAL iBind =0; iBind < pBinding->cBindings; iBind++)
	{
		...
		if (bReading)
		{
			...
			if (dbStat == DBSTATUS_S_ISNULL)
			{
				...
				// Set the length to 0 as required by the spec.
				if (pBindCur->dwPart & DBPART_LENGTH)
		...
		else
		{
			...
			if (pBindCur->dwPart & DBPART_STATUS)
			{
				...
				case DBSTATUS_S_OK:
					...
					// integrity constraints to observe
...
public:
		...
		_Out_opt_ HROW* phRow)
	{
		...
		_ATL_IF_EXISTS_FIRE_ON_ROW_CHANGE
		{
			...
			if (!pT->IncrementMutex())
			{
				// Note, we can't set this above because we may inadvertently
		...
		// Add in the storage and linkage for the row
		...
		_Outptr_result_buffer_(*pcRowsObtained) HROW **prghRows)
	{
		...
		_ATL_IF_EXISTS_FIRE_ON_ROW_CHANGE
		{
			...
			if (!pT->IncrementMutex())
			{
				...
				// inadvertently reset somebody else's pcRowsObtained
...
public:
	...
	virtual ~IObjectWithSiteSessionImpl()
	{
		...
		{
			if (SUCCEEDED(this->m_spUnkSite->QueryInterface(IID_IInternalConnection, (void**)&pConn)))
...
public:
		...
		_In_opt_ void* pSrcData)
	{
		...
		// immediate mode call FlushData.  Otherwise, add the original data
		// to the cache
		CComVariant varDeferred;
		bool bDeferred;
		hr = pT->GetPropValue(&DBPROPSET_ROWSET, DBPROP_IRowsetUpdate,
		...
		_Outptr_opt_result_buffer_maybenull_(*pcRowsUndone) DBROWSTATUS **prgRowStatus)
	{
		...
		for (ULONG ulUndoRow = 0; ulUndoRow < ulRows; ulUndoRow++)
		{
			...
			default:							// Row has been changed
				...
				// Check if we need to release the row because its ref was 0
		...
		_Outptr_opt_result_buffer_maybenull_(*pcRows) DBROWSTATUS **prgRowStatus)
	{
		...
		// temporary variables to allocate memory on heap.  As soon as we exit the function, the memory will
		// be cleaned up, just as if we were using alloca. So now, instead of calling alloca, I'll alloc
		// memory on heap using the two smart pointers below, and then assign it to the actual pointers.
