...
#pragma warning(disable : 4127 _ATLMFC_DISABLED_WARNINGS)
...
protected:
	...
	DWORD ThreadProc() throw()
	{
		...
		// thread handle is closed
		{
			...
			while (GetQueuedCompletionStatus(m_hRequestQueue, &dwBytesTransferred, &dwCompletionKey, &pOverlapped, INFINITE))
			{
...
protected:
	...
	DWORD WorkerThreadProc()
	{
		...
		while (TRUE)
		{
			...
			{
				// execute the appropriate client
