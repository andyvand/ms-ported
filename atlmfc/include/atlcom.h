...
// override it in your class and call each base class' version of this
#define BEGIN_COM_MAP(x) public: \
	typedef x _ComMapClass2; \
	static HRESULT WINAPI _Cache(_In_ void* pv, _In_ REFIID iid, _COM_Outptr_result_maybenull_ void** ppvObject, _In_ DWORD_PTR dw) throw()\
	{\
		_ComMapClass2* p = (_ComMapClass2*)pv;\
		p->Lock();\
		HRESULT hRes = E_FAIL; \
        hRes = ATL::CComObjectRootBase::_Cache(pv, iid, ppvObject, dw);\
        p->Unlock();\
...
#define COM_INTERFACE_ENTRY_IMPL(x)\
	COM_INTERFACE_ENTRY_IID(_ATL_IIDOF(x), x##Impl<_ComMapClass2>)

#define COM_INTERFACE_ENTRY_IMPL_IID(iid, x)\
	COM_INTERFACE_ENTRY_IID(iid, x##Impl<_ComMapClass2>)
//
...
#define COM_INTERFACE_ENTRY2(x, x2)\
	{&_ATL_IIDOF(x),\
	ATL::AtlVerifyInheritance<x, x2>::value + (reinterpret_cast<DWORD_PTR>(static_cast<x*>(static_cast<x2*>(reinterpret_cast<_ComMapClass2*>(_ATL_PACKING))))-_ATL_PACKING),\
	_ATL_SIMPLEMAPENTRY},
...
#define COM_INTERFACE_ENTRY_CACHED_TEAR_OFF(iid, x, punk)\
	...
	(DWORD_PTR)&ATL::_CComCacheData<\
		...
		(DWORD_PTR)offsetof(_ComMapClass2, punk)\
		>::data,\
	_Cache},
...
#define COM_INTERFACE_ENTRY_AUTOAGGREGATE(iid, punk, clsid)\
	{&iid,\
	(DWORD_PTR)&ATL::_CComCacheData<\
		ATL::CComAggregateCreator<_ComMapClass2, &clsid>,\
		(DWORD_PTR)offsetof(_ComMapClass2, punk)\
		>::data,\
	_Cache},
...
#define COM_INTERFACE_ENTRY_CHAIN(classname)\
	{NULL,\
	(DWORD_PTR)&ATL::_CComChainData<classname, _ComMapClass2>::data,\
...
public:
	...
	virtual ~CComObject()
	{
		...
		this->FinalRelease();
#ifndef __MINGW32__
#if defined(_ATL_DEBUG_INTERFACES)
		_AtlDebugInterfacesModule.DeleteNonAddRefThunk(_GetRawUnknown());
#endif
#endif
#endif // ^^^ !defined(_ATL_STATIC_LIB_IMPL) ^^^
	}
...
public:
	...
	{
#ifdef _ATL_STATIC_LIB_IMPL
		static_assert(false, "Should not be instantiated for the ATL static lib");
#else // ^^^ defined(_ATL_STATIC_LIB_IMPL) / !defined(_ATL_STATIC_LIB_IMPL) vvv
		this->m_dwRef = -(LONG_MAX/2);
		FinalRelease();
#ifndef __MINGW32__
#if defined(_ATL_DEBUG_INTERFACES)
		...
		if (ppvObj != NULL)
		{
			*ppvObj = NULL;
			...
			else
			{
				...
				hRes = _pAtlAutoThreadModule->CreateInstance((void *)m_pfnCreateInstance, riid, ppvObj);
			}
		}
		return hRes;
	}
	...
	_COM_Outptr_ IEnumConnections** ppEnum)
{
	if (ppEnum == NULL)
		return E_POINTER;
	*ppEnum = NULL;
	// Avoid instantiating CComObject in the ATL static lib during the first phase of two-phase lookup:
	using Always_CComEnumConnections = ::std::enable_if_t<!::std::is_void_v<T>, CComEnumConnections>;
	CComObject<Always_CComEnumConnections>* pEnum = NULL;
	pEnum = _ATL_NEW CComObject<Always_CComEnumConnections>;
	if (pEnum == NULL)
		return E_OUTOFMEMORY;
	T* pT = static_cast<T*>(this);
	pT->Lock();
...
ATLPREFAST_SUPPRESS(6014 6211)
		...
		_Outptr_ IEnumConnectionPoints** ppEnum)
	{
		...
		*ppEnum = NULL;
		...
		// Avoid instantiating CComObject in the ATL static lib during the first phase of two-phase lookup:
		using Always_CComEnumConnectionPoints = ::std::enable_if_t<!::std::is_void_v<T>, CComEnumConnectionPoints>;
		CComEnumConnectionPoints* pEnum = _ATL_NEW CComObject<Always_CComEnumConnectionPoints>;
		if (pEnum == NULL)
			return E_OUTOFMEMORY;
