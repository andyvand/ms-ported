...
public:
	CStringT() throw() :
		CThisSimpleString( (CAfxStringMgr*)StringTraits::GetDefaultManager() )
	{
	}
	explicit CStringT(
			_In_ CAfxStringMgr* pStringMgr) throw() :
		CThisSimpleString( (CAfxStringMgr*)pStringMgr )
	{
	}
...
public:
	...
	CStringT(
			...
			_In_ int nLength) :
		CThisSimpleString( pch, nLength, (CAfxStringMgr*)StringTraits::GetDefaultManager() )
	{
	}
	...
	CStringT(
			...
			_In_ int nLength,
			_In_ CAfxStringMgr* pStringMgr) :
		CThisSimpleString( pch, nLength, (CAfxStringMgr*)pStringMgr )
	{
	}
...
public:
	...
	CAfxStringMgr *GetManager() const throw()
	{
		CAfxStringMgr* pStringMgr = (CAfxStringMgr*)CThisSimpleString::GetManager();
		if(pStringMgr) { return pStringMgr; }

		pStringMgr = (CAfxStringMgr*)StringTraits::GetDefaultManager();
		...
		_In_opt_ va_list* pArgList)
	{
		...
		/*
			...
			So to avoid this situation use SetLastError and GetLastError to determine
...
public:
	CFixedStringMgr(
			...
			_In_ int nChars,
			_In_opt_ CAfxStringMgr* pMgr = NULL) throw() :
		m_pMgr( (CAfxStringMgr*)pMgr ),
		m_pData( pData )
	{
		m_pData->nRefs = -1;
		m_pData->nDataLength = 0;
		m_pData->nAllocLength = nChars;
		m_pData->pStringMgr = (CAfxStringMgr*)this;
...
public:
	...
	void Free(_Inout_ CStringData* pData) throw()
	{
		...
		if( pData != m_pData )
		{
			...
			pData->pStringMgr = (CAfxStringMgr*)m_pMgr;
			m_pMgr->Free( pData );
		}
		...
		_In_ int nCharSize) throw()
	{
		...
		if( pData != m_pData )
		{
			...
			{
				pNewData->pStringMgr = (CAfxStringMgr*)this;
			}
		}
		else
		{
...
public:
    ...
    CAfxStringMgr* GetBackupManager() const throw()
	{
		return m_pMgr;
	}

protected:
    CAfxStringMgr* m_pMgr;
	CStringData* m_pData;
};
...
public:
	CFixedStringT() throw() :
		...
		StringType( static_cast< CAfxStringMgr* >( this ) )
	{
	}

	explicit CFixedStringT(_In_ CAfxStringMgr* pStringMgr) throw() :
