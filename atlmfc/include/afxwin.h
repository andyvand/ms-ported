...
#pragma warning(push)
#pragma warning(disable: 4121 _ATLMFC_DISABLED_WARNINGS)
...
#define METHOD_PROLOGUE(theClass, localClass) \
	...
	(void)pThis; // avoid warning from compiler \
...
protected: \
	...
	_Ret_notnull_ static const AFX_DISPMAP* PASCAL GetThisDispatchMap(); \
...
// Window Size and Position Functions
	...
	static const CWnd wndTop; // SetWindowPos's pWndInsertAfter
    static const CWnd wndBottom; // SetWindowPos's pWndInsertAfter
    static const CWnd wndTopMost; // SetWindowPos pWndInsertAfter
    static const CWnd wndNoTopMost; // SetWindowPos pWndInsertAfter
...
protected:
	...
	/// <param name="pt">point where screen has been touched (in the client coordinates).</param>
	...
	/// <param name="ptTouch">point where screen has been touched (in the client coordinates).</param>
...
// Active Accessibility
	HRESULT get_accRole(VARIANT varChild, VARIANT *pvarRole) override;
	HRESULT get_accState(VARIANT varChild, VARIANT *pvarState) override;
	HRESULT get_accDefaultAction(VARIANT varChild, BSTR *pszDefaultAction) override;
	HRESULT accDoDefaultAction(VARIANT varChild) override;
...
extern "C" {
    extern const CRect rectDefault1;
    extern const CRect rectDefault2;
}
...
// Operations
	...
	CMenu* GetMenu() const override;
	BOOL SetMenu(CMenu* pMenu) override;
...
public:
	...
	void Dump(CDumpContext& dc) const override;
#endif
	BOOL IsFrameWnd() const override;
	BOOL OnCmdMsg(UINT nID, int nCode, void* pExtra,
		AFX_CMDHANDLERINFO* pHandlerInfo) override;
...
protected:
	...
	void PostNcDestroy() override;   // default to delete this.
...
public:
	...
	void DelayUpdateFrameMenu(HMENU hMenuAlt) override;
	CFrameWnd* GetActiveFrame() override;
	void SetMenuBarVisibility(DWORD dwStyle) override;
	BOOL SetMenuBarState(DWORD dwState) override;
...
public:
	...
	BOOL LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
					CWnd* pParentWnd, CCreateContext* pContext = NULL) override;
...
#define AFX_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES		0x20  // restoration of auto-saved documents is enabled, on restart user will be prompted to open auto-saved documents instead of last saved
...
public:
	...
	/// <returns>
	/// Application key if the function succeeds, NULL - otherwise</returns>
	/// <param name="pTM">Pointer to CAtlTransactionManager object</param>
...
// Constructors
	...
	public:
		...
		HRESULT LoadFromStream(_Inout_ IStream* pStream, _In_ DWORD grfMode) override
		{
		...
		BOOL SetChunkValue(_In_ ATL::IFilterChunkValue* pValue) override
		{
			ASSERT_VALID(m_pParentDoc);
			if (m_pParentDoc == NULL)
			{
		...
		_Success_(return != FALSE) BOOL ReadNextChunkValue(_Outptr_result_maybenull_ ATL::IFilterChunkValue** ppValue) override
		{
			ASSERT_VALID(m_pParentDoc);
			if (m_pParentDoc == NULL)
			{
		...
		void RemoveChunk(_In_ REFCLSID guid, _In_ DWORD pid) override
		{
			ASSERT_VALID(m_pParentDoc);
			if (m_pParentDoc == NULL)
			{
		...
		POSITION FindChunk(_In_ REFCLSID guid, _In_ DWORD pid) override
		{
			ASSERT_VALID(m_pParentDoc);
			if (m_pParentDoc == NULL)
			{
		...
		LPVOID GetContainer() const override
		{
			return m_pParentDoc;
		}
		...
		_Success_(return != FALSE) BOOL GetThumbnail(_In_ UINT cx, _Out_ HBITMAP* phbmp, _Out_ WTS_ALPHATYPE* pdwAlpha) override
		{
			ASSERT_VALID(m_pParentDoc);
			if (m_pParentDoc == NULL)
			{
...
// Overridables
	...
	virtual void OnDocumentEvent(int deEvent);
...
public:
	...
	/// <summary>
	/// Called by the framework when the preview handler is being unloaded.</summary>
...
public:
	...
	void CopyFrom (ATL::IFilterChunkValue* pValue) override;
	...
	HRESULT CopyChunk(STAT_CHUNK *pStatChunk) override;
	...
	CHUNKSTATE GetChunkType() const override
	{
	...
	REFGUID GetChunkGUID() const override
	{
	...
	DWORD GetChunkPID() const override
	{
	...
	HRESULT SetTextValue(REFPROPERTYKEY pkey, LPCTSTR pszValue, CHUNKSTATE chunkType = CHUNK_VALUE,
		...
		CHUNK_BREAKTYPE chunkBreakType = CHUNK_NO_BREAK) override;
	...
	HRESULT SetBoolValue(REFPROPERTYKEY pkey, BOOL bVal, CHUNKSTATE chunkType = CHUNK_VALUE, LCID locale = 0,
		DWORD cwcLenSource = 0, DWORD cwcStartSource = 0, CHUNK_BREAKTYPE chunkBreakType = CHUNK_NO_BREAK) override;
	...
	/// <param name="chunkBreakType">The type of break that separates the previous chunk from the current chunk. Values are from the CHUNK_BREAKTYPE enumeration.</param>
	/// <returns> S_OK if success; otherwise error code.</returns>
	HRESULT SetSystemTimeValue(REFPROPERTYKEY pkey, const SYSTEMTIME &systemTime, CHUNKSTATE chunkType = CHUNK_VALUE, LCID locale=0, DWORD cwcLenSource=0, DWORD cwcStartSource=0, CHUNK_BREAKTYPE chunkBreakType=CHUNK_NO_BREAK) override;
...
/*============================================================================*/
// Use version 2.1 of the RichEdit control if no version is set
