...
So we find ourselves in a quandary. For these kinds of methods, the traditional code (ATLASSERT followed by a crash), seems be the most
...
#ifndef _Out_reads_
#define _Out_reads_(X)
#endif

#ifndef _Inout_reads_
#define _Inout_reads_(X)
#endif
...
template < typename T >
_Ret_maybenull_ _Post_writable_byte_size_(dwLen) inline T* SAL_Assume_bytecap_for_opt_(
...
template < typename T >
_Ret_z_ inline T* SAL_Assume_notnull_for_opt_z_(_In_opt_z_ T* buf)
{
	ATLASSUME(buf!=0);
	return buf;
}
...
{
#ifndef _ATL_CUSTOM_THROW
static inline void WINAPI AtlThrowImpl(_In_ HRESULT hr);
#endif // _ATL_CUSTOM_THROW

static inline void WINAPI AtlThrowLastWin32();
}
