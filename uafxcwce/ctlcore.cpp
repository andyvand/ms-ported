...
//
// If the paint is a sub-classed paint, and if the display driver is a Win3.0
...
// painted and we call FillPSOnStack before calling windows with the WM_PAINT.
// This routine fills a buffer on the stack with 0x80 bytes of consecutive 
// values from 0 to 0x7f.  That way, if the  rectangle falls anywhere within 
// this buffer range, and the stack has not been modified by other means, then 
// IsRectEmpty will return FALSE, so that the control will Paint.
