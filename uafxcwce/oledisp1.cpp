...
AFX_STATIC_DATA const UINT _afxByValue[] =
{
	...
	0,
#endif
	sizeof(_STACK_INT),        // VTS_UI1
	sizeof(_STACK_INT),        // VTS_I1
	sizeof(_STACK_INT),        // VTS_UI1
	0,
	sizeof(_STACK_INT),       // VTS_UI2
...
#endif
{
	...
	*(_STACK_PTR*)pStack = (_STACK_PTR)this;
	...
	//pbParams is the order of parameters AS DEFINED!!!
	//parameters are pushed in reverse order so the order of comparison 
	//is to compare from opposite ends.
	...
	for (const BYTE* pb = pbParams; *pb != '\0'; ++pb)
	{
		...
		// by value parameters
		case VT_I1:
			*(_STACK_INT*)pStack = pArg->cVal; // 'BYTE' is passed as 'int'
		...
		case VT_UI1|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->pbVal;
			...
			break;
		case VT_I1|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->pcVal;
			pStack += sizeof(_STACK_PTR);
			break;
		case VT_UI2|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->uiVal;
			pStack += sizeof(_STACK_PTR);
			break;
		case VT_I2|VT_BYREF:
		...
		case VT_UI4|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->pulVal;
			pStack += sizeof(_STACK_PTR);
			break;
		case VT_R4|VT_BYREF:
	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
	...
	// parameter would be correctly passed in an appropriate register.
	//
	AfxRegisterState rs;

	// get the actual number of params needed to complete the call.
	 
	UINT argCount = pDispParams->cArgs + 1; //add the "this" pointer
	
	if (vtResult == VT_CY || vtResult == VT_VARIANT)
	{
		argCount++;  //add one for return
	}

	// see processor specific thunkbldr.h include  files for THUNKBUFFERSIZE
	unsigned* buffer = (unsigned*)malloc(THUNKBUFFERSIZE);
	if (buffer == NULL)
	{
		TRACE0("Error: Heap out of memory in IDispatch::Invoke!\n");
		return E_OUTOFMEMORY;
	}
	// We need to walk through our argument list to obtain the necessary
	// information.
	struct DispatchArgument *pArgTypes = (struct DispatchArgument *)_alloca(argCount * sizeof(struct DispatchArgument));

	// get to end of param type stream
	const BYTE * pbEnd = pbParams;
	while ( *pbEnd ) pbEnd++ ;
	pbEnd--;
	...
	// figure out the data for the rest
	{
		...
		index++; //get to next arg index
		}
		pbEnd--; //move to next arg type  THEY ARE IN OPPOSITE DIRECTIONS!!
	}
	// args processed, index now at the return type
	// Make sure the return parameter is dealt with correctly
	if (vtResult == VT_CY || vtResult == VT_VARIANT)
	{
		// IntType
		pArgTypes[index].ty = IntType;
	...
	if (pvarResult != NULL)
	{
		...
		case VT_I2:
			...
			break;
		case VT_UI2:
			pvarResult->uiVal = (USHORT)dwResult;
			break;
		case VT_I4:
			pvarResult->lVal = (long)dwResult;
			break;
		case VT_UI4:
			pvarResult->ulVal = (unsigned long)dwResult;
			break;
		case VT_R4:
