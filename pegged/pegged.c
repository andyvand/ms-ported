...
LPARAM       lParam)
{
	...
	case IDM_UNDO:
		...
		MessageBox(hWnd, TEXT("Can't Backup"), szAppName, MB_ICONASTERISK |
		    MB_OK);
	    break;
...
*/
	...
	case IDM_INDEX:
	    HtmlHelp(hWnd, HELP_FILE, HH_DISPLAY_INDEX, 0);
	    break;
	...
	if (ffirst) {
		...
		MessageBox(hWnd, TEXT("Not Enough Memory"), szAppName,
	...
	if ((!ffirst) && (!foomonsize))
	    if (!(hBoardBitmap = CreateCompatibleBitmap(hDC, xClient,
	         yClient))) {
		foom = 1;
		MessageBox(hWnd, TEXT("Out of Memory "), szAppName, MB_ICONASTERISK |
		    MB_OK);
		PostMessage(hWnd, WM_DESTROY, 0, 0L);
		ReleaseDC(hWnd, hDC);
		break;
	...
	if (fmovingchip) {
		...
		*/
		...
		if (fanymoremoves == 0) {/* if there aren't - declare game at an end and score */
		    ...
		    if (ntotal > 1)
			MessageBox(hWnd, TEXT("Game Over"), szAppName,
			     MB_ICONASTERISK | MB_OK);
		    else
			MessageBox(hWnd, TEXT("You Win!"), szAppName,
			     MB_ICONASTERISK | MB_OK);
	...
	*/
	...
	if (foomed) {
	    MessageBox(hWnd, TEXT("Not Enough Memory to Re-size Window"),
	...
	switch (wprevmenuitem) {
	case 3:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("3"), TEXT("entpack.ini")  );
	    break;

	case 4:
	    WritePrivateProfileString( TEXT("Pegged"), TEXT("Option"),
	         TEXT("4"), TEXT("entpack.ini")  );
	    break;

	case 5:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("5"), TEXT("entpack.ini")  );
	    break;

	case 6:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("6"), TEXT("entpack.ini")  );
	    break;

	case 7:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("7"), TEXT("entpack.ini"));
	    break;

	case 8:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("8"), TEXT("entpack.ini"));
	    break;

	case 9:
	    WritePrivateProfileString(TEXT("Pegged"), TEXT("Option"),
	         TEXT("9"), TEXT("entpack.ini"));
	    break;
