...
#ifndef NLS_MB_CODE_PAGE_TAG
#define NLS_MB_CODE_PAGE_TAG NlsMbCodePageTag
#endif

#ifndef RtlUnicodeStringToAnsiSize
                ...
                )
{
    ...
    LPSTR lpSubBlock = NULL;
    LPSTR lpSubBlockOrg = NULL;
    NTSTATUS Status = 0;
    UINT uLen = 0;

    VERBLOCK16 *pBlock = (VERBLOCK16*)pb;
    LPSTR lpStart, lpEndBlock, lpEndSubBlock;
    CHAR cTemp = 0, cEndBlock = 0;
    BOOL bLastSpec = FALSE;
    DWORD dwHeadLen = 0, dwTotBlockLen = 0;
    INT  nCmp = 0;
    DWORD LastError = ERROR_SUCCESS;

    BOOL bThunkNeeded = FALSE;
              ...
              )
{
    ANSI_STRING AnsiString;
    UNICODE_STRING UnicodeString;
    LPWSTR lpSubBlockOrg = NULL;
    LPWSTR lpSubBlock = NULL;
    NTSTATUS Status;

    VERBLOCK *pBlock = (PVOID)pb;
    LPWSTR lpStart, lpEndBlock, lpEndSubBlock;
    WCHAR cTemp = 0, cEndBlock = 0;
    DWORD dwHeadLen = 0, dwTotBlockLen = 0;
    BOOL bLastSpec;
    INT nCmp;
    BOOL bString;
    ...
    *puLen = 0;
    ...
    if (((VERHEAD*)pb)->wType)
        return VerpQueryValue16((const LPVOID)pb,
                                lpSubBlockX,
                                nIndex,
                                lplpKey,
                                lplpBuffer,
    ...
    *lpEndBlock = cEndBlock;
    ...
    if (!bUnicodeNeeded) {
        ...
        if (bString && *puLen != 0) {
            ...
            AnsiString.Buffer = (PCHAR)pb + DWORDUP(*((WORD*)pb)) + cb2;

            AnsiString.MaximumLength = (USHORT)RtlUnicodeStringToAnsiSize(&UnicodeString);
            if ( AnsiString.MaximumLength > USHORT_MAX ) {
                LastError = ERROR_INVALID_DATA;
                goto Fail;
            }
        ...
        if (lplpKey) {
            ...
            // Thunk the key
            //
            dwHeadLen = (DWORD)wcslen(*lplpKey);
