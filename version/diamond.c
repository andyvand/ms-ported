        ...
        case fdintCOPY_FILE:
            ...
            //
            {
                ...
                if (Info->RenameTargetFile) {
                    ...
                    //
                    if ((p = StringRevChar(Info->TargetFileName,'\\')) != NULL) {
                        p++;
                    } else {
                    ...
                    //
                    if ((q = StringRevChar(Parameters->psz1,'\\')) != NULL) {
                        q++;
                    } else {
                        q = Parameters->psz1;
                    }
                 ...
                 )
{
    ...
    _snprintf_s(TargetFileName, sizeof(TargetFileName), sizeof(TargetFileName), "%ls", TargetFileNameT);
#else
    snprintf(TargetFileName, sizeof(TargetFileName), "%ls", TargetFileNameT);
#endif
...
#ifdef __MINGW32__
        if (InterlockedExchangeAdd((__LONG32 volatile *)&cCabinetLoad, 1) != 0) {
#else
        if (InterlockedExchangeAdd((LONG32 *)&cCabinetLoad, 1) != 0) {
#endif
