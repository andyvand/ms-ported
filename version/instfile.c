...
#endif
{
...
#ifdef __MINGW32__
        if (InterlockedExchangeAdd((__LONG32 volatile *)&cLz32Load, 1) != 0) {
#else
        if (InterlockedExchangeAdd((LONG32 *)&cLz32Load, 1) != 0) {
#endif
     ...
     */
    MakeFileName(szSrcFile, (LPWSTR)lpszSrcDir, (LPWSTR)lpszSrcFileName, ARRAYSIZE(szSrcFile));
   ...
   */
     ...
     )
   {
     ...
     */
    wDirLen = (WORD)MakeFileName(szDstFile, (LPWSTR)lpszDstDir, (LPWSTR)lpszDstFileName, ARRAYSIZE(szDstFile));
    ...
    if ((wFlags&VIFF_FORCEINSTALL) && *lpszTmpFile) {
        ...
        MakeFileName(szSrcFile, (LPWSTR)lpszDstDir, lpszTmpFile, ARRAYSIZE(szSrcFile));
        LogData("inst", __LINE__, (DWORD)puTmpFileLen);
    ...
    if (!HIWORD(dwSrcAttr)) {
        ...
        if (!(wFlags & VIFF_FORCEINSTALL) &&
            ...
            if (!(pSrcVer=MyGetFileVersionInfo(szSrcFile))) {
                ...
                if (VerQueryValueW(pDstVer, szTrans, (LPVOID)&lpDstTrans, (PUINT)&dwDstNum) &&
                    VerQueryValueW(pSrcVer, szTrans, (LPVOID)&lpSrcTrans, (PUINT)&dwSrcNum)) {
         ...
         */
        LogData("inst", __LINE__, (DWORD)puTmpFileLen);
        if (FileInUse(szDstFile, (LPWSTR)lpszDstFileName)) {
            LogData("inst", __LINE__, (DWORD)puTmpFileLen);
    ...
    if (dwRetVal) {
        ...
        *puTmpFileLen = wTemp + 1;
        ...
        if (!(wFlags&VIFF_DONTDELETEOLD) &&
            ...
            MakeFileName(szCurFile, (LPWSTR)lpszCurDir, (LPWSTR)lpszDstFileName, ARRAYSIZE(szCurFile));
            if (!HIWORD(GetFileAttributes(szCurFile)) &&
                (FileInUse(szCurFile, (LPWSTR)lpszDstFileName) ||
