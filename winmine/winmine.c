...
VOID Error(TCHAR *sz)
{
    MessageBox(hwndMain, (LPTSTR)sz, (LPTSTR)TEXT("Minesweeper error"), MB_OK|MB_ICONEXCLAMATION);
}

INT CchString(TCHAR *sz, INT ids)
{
    return LoadString(hInst, (WORD)ids, (LPTSTR)sz, 255);
}
...
VOID DoHelp(INT idContext)
{
    ...
    LoadString(hInst, (WORD)idsHelpFile, (LPTSTR)sz, 100);
